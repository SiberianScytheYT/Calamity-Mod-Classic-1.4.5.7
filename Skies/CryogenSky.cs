...
namespace CalRD.Skies
{
    public class CryogenSky : CustomSky
    {
        ...
        internal static Texture2D AuroraTexture => ModContent.Request<Texture2D>("CalRD/ExtraTextures/AuroraTexture").Value;
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < Auroras.Length; i++)
            {
                ...
                Auroras[i].CenterOffsetRatio += 1f / 1800f * MathHelper.Lerp(0.2f, 1f, 1f - GetCryogenLifeRatio()) * (float)Math.Sin(Main.GlobalTimeWrappedHourly * 0.9f);
            }
        }
        ...
        public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
        {
            if (maxDepth >= float.MaxValue && minDepth < float.MaxValue)
            {
                ...
                for (int i = 0; i < Auroras.Length; i++)
                {
                    ...
                    if (fadeIn > 0f)
                        auroraColor *= Utils.GetLerpValue(45f, 0f, fadeIn);
                    if (fadeOut > 0f)
                        auroraColor *= Utils.GetLerpValue(0f, 45f, fadeOut);
                    if (Main.dayTime)
                        auroraColor *= 0.4f;
