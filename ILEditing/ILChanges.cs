...
namespace CalRD.ILEditing
{
    public class ILChanges
    {
        ...
        private static void ApplyBossZenDuringSlimeRain() => Terraria.On_NPC.SlimeRainSpawns += PreventBossSlimeRainSpawns;
        ...
        private static void SlideDungeonOver()
        {
            Terraria.IL_WorldGen.MakeDungeon += (il) =>
            {
                ...
                {
                    GenVars.dungeonX += (GenVars.dungeonX < Main.maxTilesX / 2).ToDirectionInt() * 450;
                    GenVars.dungeonX = Utils.Clamp(GenVars.dungeonX, 200, Main.maxTilesX - 200);
                });
            };
        }
        ...
        {
            Terraria.IL_WorldGen.GrowLivingTree += (il) =>
            {
                var cursor = new ILCursor(il);
        ...
        {
            Terraria.IL_Main.UpdateTime += (il) =>
            {
                ...
                {
                    CalRD.Instance.Logger.Warn("Town NPC spawn editing code failed.");
                    return;
                }
            ...
            {
                int oldWorldRate = (int)Main.desiredWorldTilesUpdateRate;
