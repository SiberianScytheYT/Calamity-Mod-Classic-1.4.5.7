...
using CalRD.Items.Fishing.SulphurCatches;
...
namespace CalRD.CalPlayer
{
    ...
    {
        #region Catch Fish
        public static void CalamityCatchFish(Player player, FishingAttempt attempt, ref int itemDrop, ref int npcSpawn, ref AdvancedPopupRequest sonar, ref Vector2 sonarPosition)
        {
			...
			// Old Duke spawn
			if (canSulphurFish && bait == ModContent.ItemType<BloodwormItem>() && water)
			{
				...
				if (Main.rand.NextBool(chanceForPotionFish))
				{
					...
					else if (water)
					{
						if (player.ZoneDirtLayerHeight || player.ZoneRockLayerHeight)
						{
							...
							CalamityUtils.AddWithCondition<int>(fishList, ItemID.ChaosFish, player.ZoneHallow);
						...
						CalamityUtils.AddWithCondition<int>(fishList, ItemID.PrincessFish, player.ZoneHallow);
						CalamityUtils.AddWithCondition<int>(fishList, ItemID.Prismite, player.ZoneHallow);
				...
				if (water)
				{
					if (Main.rand.Next(100) < chanceForCrates)
					{
						if (Main.rand.NextBool(chanceForRareItems) && modPlayer.enchantedPearl && modPlayer.fishingStation && player.cratePotion)
						{
							...
							CalamityUtils.AddWithCondition<int>(rareItemList, ItemID.CrystalSerpent, player.ZoneHallow);
						...
						else if (Main.rand.NextBool(chanceForBiomeCrate))
						{
							...
							CalamityUtils.AddWithCondition<int>(biomeCrateList, ItemID.HallowedFishingCrate, player.ZoneHallow);
        ...
        public static void CalamityGetFishingLevel(Player player, ref Item fishingRod, ref Item bait, ref float fishingLevel)
        {
            CalamityPlayer modPlayer = player.Calamity();
