...
using CalRD.Items.Fishing.FishingRods;
...
namespace CalRD.CalPlayer
{
	public class CalamityPlayerMiscEffects
	{
					...
					if (CalamityConfig.Instance.Rippers)
					{
						...
						if (modPlayer.rage >= modPlayer.rageMax)
						{
							...
							if (modPlayer.playFullRageSound)
							{
								...
								SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/FullRage"), player.position);
							}
						}
						else
						...
						if (modPlayer.adrenaline >= modPlayer.adrenalineMax)
						{
							...
							if (modPlayer.playFullAdrenalineSound)
							{
								...
								SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/FullAdrenaline"), player.position);
							}
						}
						else
		...
		private static void MiscEffects(Player player, CalamityPlayer modPlayer, Mod mod)
		{
			...
			if (player.whoAmI == Main.myPlayer)
			{
				if (CalamityWorld.rainingAcid && modPlayer.ZoneSulphur && !CalamityPlayer.areThereAnyDamnBosses && player.Center.Y < Main.worldSurface * 16f + 800f)
				{
					...
					if (player.miscCounter % slimeRainRate == 0f)
					{
						...
						{
							NPC.NewNPC(player.GetSource_FromThis(), (int)spawnPoint.X, (int)spawnPoint.Y, ModContent.NPCType<IrradiatedSlime>());
						}
					}
				}
			}
			...
			if (player.whoAmI == Main.myPlayer)
			{
				if (modPlayer.hydrothermalSmoke)
				{
					...
					{
						Projectile.NewProjectile(player.GetSource_FromThis(), player.Center, Vector2.Zero, ModContent.ProjectileType<HydrothermalSmoke>(), 0, 0f, player.whoAmI);
					}
				}
			...
			if (CalamityWorld.death)
			{
				if (player.whoAmI == Main.myPlayer)
				{
					...
					Collision.HurtTile tileType;
					if (!player.mount.Active || !player.mount.Cart)
						tileType = Collision.HurtTiles(player.position, player.width, player.height, player);
					else
						tileType = Collision.HurtTiles(player.position, player.width, player.height - 16, player);
					...
					if (!BossRushEvent.BossRushActive && !CalamityPlayer.areThereAnyDamnBosses && !CalamityPlayer.areThereAnyDamnEvents)
					{
						...
						// Calculate underground darkness here. The effect is applied in CalRD.ModifyLightingBrightness.
							...
							if (Main.raining)
							{
								...
								if (player.ZoneSnow)
								{
									if (!tileSafely.HasTile)
									{
										...
										float windVelocity = (float)Math.Sqrt(Math.Abs(Main.windSpeedCurrent)) * Math.Sign(Main.windSpeedCurrent) * (Main.cloudAlpha + 0.5f) * 25f + Main.rand.NextFloat() * 0.2f - 0.1f;
								...
								else
								{
									if (player.ZoneBeach && !modPlayer.ZoneSulphur)
									{
										...
										if (player.miscCounter == 280 && Main.rand.NextBool(randomFrequency) && player.ownedProjectileCounts[ProjectileID.Cthulunado] < 1)
										{
											...
											int tornado = Projectile.NewProjectile(player.GetSource_FromThis(), x * 16 + 8, y * 16 - 24, 0f, 0f, ProjectileID.Cthulunado, 50, 4f, player.whoAmI, 16f, 24f);
						...
						bool immunityToCold = player.HasBuff(BuffID.Campfire) || Main.SceneMetrics.HasCampfire || player.resistCold || modPlayer.eskimoSet || player.buffImmune[BuffID.Frozen] || modPlayer.aAmpoule || player.HasBuff(BuffID.Inferno) || immunityToHotAndCold || modPlayer.externalColdImmunity;
			...
			if (modPlayer.omegaBlueSet)
			{
				...
				if (player.ownedProjectileCounts[ModContent.ProjectileType<OmegaBlueTentacle>()] < 6 && Main.myPlayer == player.whoAmI)
				{
					...
					for (int i = 0; i < 6; i++)
					{
						if (!tentaclesPresent[i])
						{
							...
							Projectile.NewProjectile(player.GetSource_FromThis(), player.Center, vel, ModContent.ProjectileType<OmegaBlueTentacle>(), damage, 8f, Main.myPlayer, Main.rand.Next(120), i);
						}
					}
				}
				...
				player.GetDamage(DamageClass.Generic) += damageUp;
						...
						for (int j = 0; j < loopAmt; j++)
						{
							...
							if (!Collision.SolidCollision(center, num3, num3) && !Collision.WetCollision(center, num3, num3))
							{
								...
								if (Collision.CanHit(new Vector2(player.Center.X, player.position.Y), 1, 1, center, 1, 1) || Collision.CanHit(new Vector2(player.Center.X, player.position.Y - 50f), 1, 1, center, 1, 1))
								{
									...
									if (Main.rand.NextBool(3) && Main.tile[xSpawn, ySpawn] != null && Main.tile[xSpawn, ySpawn].WallType > 0)
										shouldSpawn = true;
									else
									{
									...
									if (shouldSpawn)
									{
										...
										if (shouldSpawn && Main.myPlayer == player.whoAmI)
											Projectile.NewProjectile(player.GetSource_FromThis(), center, Vector2.Zero, ModContent.ProjectileType<TheDeadlyMicrobeProjectile>(), damage, knockBack, player.whoAmI, 0f, 0f);
									}
								}
							}
						}
						...
						for (int x = i - distance; x <= i + distance; x++)
						{
							for (int y = j - distance; y <= j + distance; y++)
							{
								...
								if (Main.rand.NextBool(4))
								{
									...
									if (vector.Length() < (float)distance && x > 0 && x < Main.maxTilesX - 1 && y > 0 && y < Main.maxTilesY - 1 && tile != null && tile.HasTile)
									{
										...
										if (tile.TileType == TileID.SmallPiles && tile.TileFrameY == 18)
										{
											if (tile.TileFrameX >= 576 && tile.TileFrameX <= 882)
												shouldSpawnDust = true;
										}
										else if (tile.TileType == TileID.LargePiles && tile.TileFrameX >= 864 && tile.TileFrameX <= 1170)
											shouldSpawnDust = true;

										if (shouldSpawnDust || Main.tileSpelunker[tile.TileType] || (Main.tileAlch[tile.TileType] && tile.TileType != TileID.ImmatureHerbs))
										{
			...
			if (modPlayer.plaguedFuelPackDash > 0 && player.whoAmI == Main.myPlayer)
			{
				...
				for (int i = 0; i < numClouds; i++)
				{
					...
					int projectile = Projectile.NewProjectile(player.GetSource_FromThis(), player.Center, cloudVelocity, ModContent.ProjectileType<PlaguedFuelPackCloud>(), (int)(20 * player.RogueDamage()), 0, player.whoAmI, 0, 0);
			...
			if (modPlayer.blunderBoosterDash > 0 && player.whoAmI == Main.myPlayer)
			{
				...
				for (int i = 0; i < lightningCount; i++)
				{
					...
					int projectile = Projectile.NewProjectile(player.GetSource_FromThis(), player.Center, lightningVel, ModContent.ProjectileType<BlunderBoosterLightning>(), (int)(30 * player.RogueDamage()), 0, player.whoAmI, Main.rand.Next(2), 0f);
					Main.projectile[projectile].timeLeft = Main.rand.Next(180, 240);
				}
			...
			if (modPlayer.gSabaton && player.whoAmI == Main.myPlayer)
			{
				if (modPlayer.gSabatonCooldown <= 0 && !player.mount.Active)
				{
					if (player.controlDown && player.releaseDown && player.position.Y != player.oldPosition.Y)
					{
						...
						player.gravity *= 2f;
						Projectile.NewProjectile(player.GetSource_FromThis(), player.Center.X, player.Center.Y + (player.height / 5f), player.velocity.X, player.velocity.Y, ModContent.ProjectileType<SabatonSlam>(), 0, 0, player.whoAmI);
					}
				}
		...
		private static void MaxLifeAndManaEffects(Player player, CalamityPlayer modPlayer, Mod mod)
		{
			...
			if (modPlayer.silvaHitCounter > 0)
			{
				...
				if (player.statLifeMax2 <= 400)
				{
					...
					if (modPlayer.silvaCountdown > 0)
					{
						...
						SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/SilvaDispel"), player.position);
					}
			...
			{
				Asset<Texture2D> rain3 = ModContent.Request<Texture2D>("CalRD/ExtraTextures/Rain3");
				Asset<Texture2D> rainOriginal = ModContent.Request<Texture2D>("CalRD/ExtraTextures/RainOriginal");
				Asset<Texture2D> mana2 = ModContent.Request<Texture2D>("CalRD/ExtraTextures/Mana2");
				Asset<Texture2D> mana3 = ModContent.Request<Texture2D>("CalRD/ExtraTextures/Mana3");
				Asset<Texture2D> mana4 = ModContent.Request<Texture2D>("CalRD/ExtraTextures/Mana4");
				Asset<Texture2D> manaOriginal = ModContent.Request<Texture2D>("CalRD/ExtraTextures/ManaOriginal");
				Asset<Texture2D> carpetAuric = ModContent.Request<Texture2D>("CalRD/ExtraTextures/AuricCarpet");
				Asset<Texture2D> carpetOriginal = ModContent.Request<Texture2D>("CalRD/ExtraTextures/Carpet");
		...
		private static void OtherBuffEffects(Player player, CalamityPlayer modPlayer)
		{
			...
			if (modPlayer.etherealExtorter)
			{
				bool ZoneForest = !modPlayer.ZoneAbyss && !modPlayer.ZoneSulphur && !modPlayer.ZoneAstral && !modPlayer.ZoneCalamity &&
					!modPlayer.ZoneSunkenSea && !player.ZoneSnow && !player.ZoneCorrupt && !player.ZoneCrimson && !player.ZoneHallow &&
			...
			if (modPlayer.dAmulet)
			{
				player.panic = true;
				player.pStone = true;
				player.GetArmorPenetration(DamageClass.Generic) += modPlayer.rampartOfDeities ? 20 : 10;
			}
			...
			if (modPlayer.bloodflareSummon)
			{
				...
				if (player.whoAmI == Main.myPlayer && modPlayer.bloodflareSummonTimer <= 0)
				{
					...
					{
						float ai1 = I * 120;
						Projectile.NewProjectile(player.GetSource_FromThis(), player.Center.X + (float)(Math.Sin(I * 120) * 550), player.Center.Y + (float)(Math.Cos(I * 120) * 550), 0f, 0f,
				...
				if (modPlayer.tarraLifeAuraTimer == 0 && player.whoAmI == Main.myPlayer)
				{
					...
					for (int i = 0; i < Main.maxNPCs; ++i)
					{
						...
						if (Vector2.Distance(player.Center, npc.Center) <= range)
							Projectile.NewProjectile(player.GetSource_FromThis(), npc.Center, Vector2.Zero, ModContent.ProjectileType<DirectStrike>(), damage, 0f, player.whoAmI, i);
					}
				}
			}
				...
				if (modPlayer.navyRodAuraTimer == 0 && player.whoAmI == Main.myPlayer)
				{
					...
					for (int i = 0; i < Main.maxNPCs; ++i)
					{
						...
						if (Main.rand.NextBool(10))
						{
							...
							int spark = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, velocity, ModContent.ProjectileType<EutrophicSpark>(), damage / 2, 0f, player.whoAmI, 0f, 0f);
			...
			if (modPlayer.gArtifact)
			{
				...
				if (player.whoAmI == Main.myPlayer)
				{
					...
					if (player.ownedProjectileCounts[ModContent.ProjectileType<SonOfYharon>()] < 2)
						Projectile.NewProjectile(player.GetSource_FromThis(), player.Center, Vector2.Zero, ModContent.ProjectileType<SonOfYharon>(), (int)(232f * player.MinionDamage()), 2f, Main.myPlayer, 0f, 0f);
				}
			}
			...
			if (modPlayer.pArtifact)
			{
				if (player.whoAmI == Main.myPlayer)
				{
					...
					if (player.ownedProjectileCounts[ModContent.ProjectileType<MiniGuardianHealer>()] < 1)
						Projectile.NewProjectile(player.GetSource_FromThis(), player.Center.X, player.Center.Y, 0f, -6f, ModContent.ProjectileType<MiniGuardianHealer>(), 0, 0f, Main.myPlayer, 0f, 0f);
					...
					if (crystal || player.Calamity().minionSlotStat >= 10)
					{
						...
						if (player.ownedProjectileCounts[ModContent.ProjectileType<MiniGuardianDefense>()] < 1)
							Projectile.NewProjectile(player.GetSource_FromThis(), player.Center.X, player.Center.Y, 0f, -3f, ModContent.ProjectileType<MiniGuardianDefense>(), 1, 1f, Main.myPlayer, 0f, 0f);
					}
					...
					if (crystal || summonSet)
					{
						...
						if (player.ownedProjectileCounts[ModContent.ProjectileType<MiniGuardianAttack>()] < 1)
							Projectile.NewProjectile(player.GetSource_FromThis(), player.Center.X, player.Center.Y, 0f, -1f, ModContent.ProjectileType<MiniGuardianAttack>(), 1, 1f, Main.myPlayer, 0f, 0f);
					}
				}
			}
			...
			if (modPlayer.plaguebringerPistons)
			{
				...
				if (modPlayer.pistonsCounter % 12 == 0)
				{
					if (player.velocity.Length() >= 5f && player.whoAmI == Main.myPlayer)
					{
						...
						{
							int bee = Projectile.NewProjectile(player.GetSource_FromThis(), player.Center.X, player.Center.Y, Main.rand.NextFloat(-35f, 35f) * 0.02f, Main.rand.NextFloat(-35f, 35f) * 0.02f, (Main.rand.NextBool(4) ? ModContent.ProjectileType<PlagueBeeSmall>() : player.beeType()), damage, player.beeKB(0f), player.whoAmI, 0f, 0f);
			...
			if (modPlayer.blunderBooster)
			{
				if (player.whoAmI == Main.myPlayer)
				{
					if (player.ownedProjectileCounts[ModContent.ProjectileType<BlunderBoosterAura>()] < 1)
						Projectile.NewProjectile(player.GetSource_FromThis(), player.Center, Vector2.Zero, ModContent.ProjectileType<BlunderBoosterAura>(), (int)(30 * player.RogueDamage()), 0f, player.whoAmI, 0f, 0f);
				}
			}
			...
			if (modPlayer.tesla)
			{
				if (player.whoAmI == Main.myPlayer)
				{
					...
					if (player.ownedProjectileCounts[ModContent.ProjectileType<TeslaAura>()] < 1)
						Projectile.NewProjectile(player.GetSource_FromThis(), player.Center, Vector2.Zero, ModContent.ProjectileType<TeslaAura>(), (int)(10 * player.AverageDamage()), 0f, player.whoAmI, 0f, 0f);
				}
			}
			...
			if (modPlayer.prismaticLasers > 1800 && player.whoAmI == Main.myPlayer)
			{
				...
				for (int index = 0; index < laserAmt; index++)
				{
					...
					int laser = Projectile.NewProjectile(player.GetSource_FromThis(), startPos, velocity, ModContent.ProjectileType<MagicNebulaShot>(), dmg, 4f, player.whoAmI, 0f, 0f);
			...
			if (modPlayer.theBee)
			{
				if (player.statLife >= player.statLifeMax2)
				{
					...
					player.GetDamage(DamageClass.Generic) += beeBoost;
				}
			}
		...
		{
			float allDamageStat = player.GetDamage(DamageClass.Generic).Additive - 1f;
			modPlayer.actualMeleeDamageStat = player.GetDamage(DamageClass.Melee).Additive + allDamageStat;
			modPlayer.damageStats[0] = (int)((player.GetDamage(DamageClass.Melee).Additive + allDamageStat - 1f) * 100f);
			modPlayer.damageStats[1] = (int)((player.GetDamage(DamageClass.Ranged).Additive + allDamageStat - 1f) * 100f);
			modPlayer.damageStats[2] = (int)((player.GetDamage(DamageClass.Magic).Additive + allDamageStat - 1f) * 100f);
			modPlayer.damageStats[3] = (int)((player.GetDamage(DamageClass.Summon).Additive + allDamageStat - 1f) * 100f);
			...
			modPlayer.critStats[0] = (int)player.GetCritChance(DamageClass.Melee);
			modPlayer.critStats[1] = (int)player.GetCritChance(DamageClass.Ranged);
			modPlayer.critStats[2] = (int)player.GetCritChance(DamageClass.Magic);
			modPlayer.critStats[3] = (int)player.GetCritChance(DamageClass.Throwing) + modPlayer.throwingCrit;
			...
			modPlayer.meleeSpeedStat = (int)((1f - player.GetAttackSpeed(DamageClass.Melee)) * (100f / player.GetAttackSpeed(DamageClass.Melee)));
			...
			modPlayer.armorPenetrationStat = (int)player.GetArmorPenetration(DamageClass.Generic);
