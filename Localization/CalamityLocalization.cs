    ...
    public class CalamityLocalization
    {
        ...
        */
        ...
        public static void AddLocalizations()
        {
            ...
            {
                LocalizedText text = Language.GetOrRegister(ModContent.GetInstance<CalRD>(), localization[0]);
                // text.SetDefault(localization[1]);
                //ModContent.GetInstance<CalRD>().AddTranslation(text) /tModPorter Note: Removed. Use Language.GetOrRegister/;
            }
            Unload();
        }
        */
