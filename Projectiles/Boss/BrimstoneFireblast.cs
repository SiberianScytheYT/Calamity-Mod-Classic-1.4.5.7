...
namespace CalRD.Projectiles.Boss
{
    public class BrimstoneFireblast : ModProjectile
    {
        ...
        {
			SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/SCalSounds/BrimstoneGigablastImpact"), Projectile.Center);
			...
			if (Projectile.ai[1] == 0f)
			{
				...
				if (Projectile.owner == Main.myPlayer)
				{
					for (int i = 0; i < 30; i++)
					{
						...
						Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)(Math.Sin(offsetAngle) * 5f), (float)(Math.Cos(offsetAngle) * 5f), ModContent.ProjectileType<BrimstoneBarrage>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 1f);
						Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)(-Math.Sin(offsetAngle) * 5f), (float)(-Math.Cos(offsetAngle) * 5f), ModContent.ProjectileType<BrimstoneBarrage>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 1f);
					}
				}
			}
