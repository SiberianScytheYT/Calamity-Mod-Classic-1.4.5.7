...
namespace CalRD.Projectiles.Boss
{
    public class DeusRitualDrama : ModProjectile
    {
		...
		public override void AI()
		{
			...
			{
				int idx = NPC.NewNPC(Entity.GetSource_FromThis(), (int)Projectile.Center.X, (int)Projectile.Center.Y - (int)MaxUpwardRise, ModContent.NPCType<AstrumDeusHeadSpectral>(), 1);
				if (idx != -1)
				{
					SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/AstrumDeusSpawn"), Projectile.Center);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (!Main.dedServ && Math.Abs(offset.X) < 6f && Time > 60f)
            {
				...
				SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/AstralBeaconOrbPulse"), Projectile.Center);
            }
