...
namespace CalRD.Projectiles.Boss
{
    ...
    {
        public override string Texture => "CalRD/Projectiles/LightningProj";
					...
					if (Projectile.ai[1] >= 90f)
					{
						if (Projectile.owner == Main.myPlayer)
						{
							...
							for (int i = 0; i < totalProjectiles; i++)
							{
								...
								Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, vector, Projectile.type, Projectile.damage, 0f, Main.myPlayer, Main.rand.Next(6), 0f);
							}
						}
				...
				case 7:
					...
					if (Projectile.ai[1] >= 120f)
					{
						if (Projectile.owner == Main.myPlayer)
						{
							...
							{
								Vector2 vector = new Vector2(0f, -8f).RotatedBy(radians * i);
								Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, vector, Projectile.type, Projectile.damage, 0f, Main.myPlayer, Main.rand.Next(2), 0f);
							}
						}
					...
					if (Projectile.localAI[0] >= 180f)
					{
						if (Projectile.owner == Main.myPlayer)
						{
							...
							for (int i = 0; i < totalProjectiles; i++)
							{
								...
								int proj = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, vector + vector2, Projectile.type, Projectile.damage, 0f, Main.myPlayer, 8f, 0f);
