...
namespace CalRD.Projectiles.Boss
{
	public class BirbAura : ModProjectile
    {
		...
		public override void ModifyHitPlayer(Player target, ref Player.HurtModifiers modifiers)
		{
			if (timer <= 0f && (Projectile.localAI[0] >= 120f || Projectile.timeLeft <= 900))
			{
				if (Projectile.owner == Main.myPlayer)
				{
					...
					SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/LightningStrike"), new Vector2(target.Center.X, target.Center.Y - 300));
					Vector2 ai0 = target.Center - fireFrom;
					float ai = Main.rand.Next(100);
					Vector2 velocity = Vector2.Normalize(ai0.RotatedByRandom(MathHelper.PiOver4)) * 7f;
					Projectile.NewProjectile(Entity.GetSource_FromThis(), fireFrom.X, fireFrom.Y, velocity.X, velocity.Y, ModContent.ProjectileType<RedLightning>(), Projectile.damage, 0f, Projectile.owner, ai0.ToRotation(), ai);
				}
			}
			Projectile.damage = 1;
		}
		...
		public override bool PreDraw(ref Color lightColor)
        {
			...
			Texture2D texture2 = ModContent.Request<Texture2D>("CalRD/ExtraTextures/Lasers/BirbAuraStart").Value;
			Texture2D texture3 = ModContent.Request<Texture2D>("CalRD/ExtraTextures/Lasers/BirbAuraEnd").Value;
			float num223 = Projectile.localAI[1]; //length of laser
			Color color44 = new Color(128, 128, 128, 0);
			Vector2 vector = Projectile.Center - Main.screenPosition;
			Rectangle? sourceRectangle2 = null;
			Main.EntitySpriteDraw(texture2, vector, sourceRectangle2, color44, Projectile.rotation, texture2.Size() / 2f, Projectile.scale, SpriteEffects.None, 0f);
			...
			if (num223 > 0f)
            {
                ...
                while (num224 + 1f < num223)
                {
                    ...
                    Main.EntitySpriteDraw(texture, value20 - Main.screenPosition, new Rectangle?(rectangle7), color44, Projectile.rotation, new Vector2(rectangle7.Width / 2, 0f), Projectile.scale, SpriteEffects.None, 0f);
			...
			Main.EntitySpriteDraw(texture3, vector2, sourceRectangle2, color44, Projectile.rotation, texture3.Frame(1, 1, 0, 0).Top(), Projectile.scale, SpriteEffects.None, 0f);
			return false;
        }
