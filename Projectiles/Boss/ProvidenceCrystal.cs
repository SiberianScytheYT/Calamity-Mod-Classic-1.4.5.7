				...
				if (Projectile.localAI[0] % 30f == 0f || Main.dayTime)
				{
					...
					if (Projectile.owner == Main.myPlayer)
					{
						...
						for (int i = 0; i < totalProjectiles; i++)
						{
							...
							Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, speedX + speedAdjustment * i + randomSpread, speedY, ModContent.ProjectileType<ProvidenceCrystalShard>(), Projectile.damage, Projectile.knockBack, Projectile.owner, x4, Projectile.whoAmI);
						}
					}
