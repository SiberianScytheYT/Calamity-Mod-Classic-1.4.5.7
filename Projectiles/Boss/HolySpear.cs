        ...
        public override bool PreDraw(ref Color lightColor)
        {
			...
			if (CalamityConfig.Instance.Afterimages)
			{
				for (int i = 0; i < Projectile.oldPos.Length; i++)
				{
					...
					Main.EntitySpriteDraw(value, drawPos, null, color, Projectile.rotation, origin5, vector29, spriteEffects, 0f);
					Main.EntitySpriteDraw(value, drawPos, null, color, Projectile.rotation, origin5, vector30, spriteEffects, 0f);

					color = Projectile.GetAlpha(color35) * ((Projectile.oldPos.Length - i) / Projectile.oldPos.Length);
					Main.EntitySpriteDraw(value, drawPos, null, color, Projectile.rotation, origin5, vector29 * 0.6f, spriteEffects, 0f);
					Main.EntitySpriteDraw(value, drawPos, null, color, Projectile.rotation, origin5, vector30 * 0.6f, spriteEffects, 0f);
				}
			}
