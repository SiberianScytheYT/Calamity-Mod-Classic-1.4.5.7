...
namespace CalRD.Projectiles.Boss
{
    public class BrimstoneGigaBlast : ModProjectile
    {
        ...
        {
			SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/SCalSounds/BrimstoneFireblastImpact"), Projectile.Center);
			...
			if (Projectile.ai[1] == 0f)
			{
				...
				if (Projectile.owner == Main.myPlayer)
				{
					for (int i = 0; i < 8; i++)
					{
						...
						Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)(Math.Sin(offsetAngle) * 7f), (float)(Math.Cos(offsetAngle) * 7f), ModContent.ProjectileType<BrimstoneBarrage>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 1f);
						Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)(-Math.Sin(offsetAngle) * 7f), (float)(-Math.Cos(offsetAngle) * 7f), ModContent.ProjectileType<BrimstoneBarrage>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 1f);
					}
				}
			}
