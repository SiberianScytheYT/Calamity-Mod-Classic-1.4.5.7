...
namespace CalRD.Projectiles.Boss
{
    public class HolyFire : ModProjectile
    {
        ...
        {
            Texture2D texture2D13 = Main.dayTime ? TextureAssets.Projectile[Projectile.type].Value : ModContent.Request<Texture2D>("CalRD/Projectiles/Boss/HolyFireNight").Value;
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (randomShot == 0)
            {
                ...
                {
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, 0.01f, 0f, ModContent.ProjectileType<HolyFire2>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, -0.01f, 0f, ModContent.ProjectileType<HolyFire2>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                }
            }
            else
            {
                if (Projectile.owner == Main.myPlayer)
                {
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, 0.05f, 0f, ModContent.ProjectileType<HolyFire2>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, -0.05f, 0f, ModContent.ProjectileType<HolyFire2>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                }
            }
