...
namespace CalRD.Projectiles.Boss
{
    ...
    {
        public override string Texture => "CalRD/Projectiles/Boss/OldDukeVortex";
		...
		public override void AI()
        {
			...
			else if (Projectile.ai[0] < 480f)
			{
				if (Projectile.ai[0] % 10f == 9f)
				{
					...
					Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Top + new Vector2(Main.rand.NextFloat(-80f, 80f), 100f), velocity, 
				...
				if (Projectile.ai[0] % 35f == 34f)
				{
					...
					Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Top + new Vector2(Main.rand.NextFloat(-30f, 30f), 100f), velocity,
			...
			if (Projectile.ai[0] >= 600f)
			{
				...
				if (Projectile.ai[0] == 660f)
				{
					if (canSpawnBoomer)
					{
						...
						{
							int boomer = NPC.NewNPC(Entity.GetSource_FromThis(), (int)Projectile.Top.X, (int)Projectile.Top.Y + 100, ModContent.NPCType<OldDuke>());
