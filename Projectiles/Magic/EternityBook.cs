...
namespace CalRD.Projectiles.Magic
{
	public class EternityBook : ModProjectile
    {
        ...
        {
            Projectile hex = Projectile.NewProjectileDirect(Entity.GetSource_FromThis(), target.Center, Vector2.Zero, ModContent.ProjectileType<EternityHex>(), Projectile.damage, 0f, owner.whoAmI, target.whoAmI);
            hex.localAI[1] = Projectile.whoAmI;

            for (int i = 0; i < 5; i++)
            {
                float crystalAngleOffset = MathHelper.TwoPi / 5f * i;
                Projectile crystal = Projectile.NewProjectileDirect(Entity.GetSource_FromThis(), target.Center, Vector2.Zero, ModContent.ProjectileType<EternityCrystal>(), 0, 0f, owner.whoAmI, target.whoAmI, crystalAngleOffset);
                crystal.frame = i % 2;
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                Projectile circleSpell = Projectile.NewProjectileDirect(Entity.GetSource_FromThis(), target.Center, Vector2.Zero, ModContent.ProjectileType<EternityCircle>(), 0, 0f, owner.whoAmI, target.whoAmI, circleOffset);
