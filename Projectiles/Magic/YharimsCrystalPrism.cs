            ...
            if (Projectile.owner == Main.myPlayer)
            {
                ...
                if (crystalStillInUse && Projectile.ai[0] == 1f)
                {
                    ...
                    for (int b = 0; b < NumBeams; b++)
                        Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, beamVelocity, ModContent.ProjectileType<YharimsCrystalBeam>(), damage, kb, Projectile.owner, b, Projectile.GetByUUID(Projectile.owner, Projectile.whoAmI));
