...
namespace CalRD.Projectiles.Magic
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/Magic/DarkSpark";
            ...
            if (flag10 && Main.myPlayer == Projectile.owner)
            {
                ...
                if (flag12)
                {
                    if (Projectile.ai[0] == 1f)
                    {
                        ...
                        for (int l = 0; l < 7; l++)
                        {
                            Projectile.NewProjectile(Entity.GetSource_FromThis(), center3.X, center3.Y, vector12.X, vector12.Y, ModContent.ProjectileType<DarkSparkBeam>(), num29, Projectile.knockBack, Projectile.owner, (float)l, Projectile.GetByUUID(Projectile.owner, Projectile.whoAmI));
                        }
                        Projectile.netUpdate = true;
                    }
                }
