            ...
            if (target.life <= 0)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
					for (int s = 0; s < shardAmt; s++)
					{
						...
						int shard = Projectile.NewProjectile(Entity.GetSource_FromThis(), target.Center, velocity, ModContent.ProjectileType<AquashardSplit>(), shardDamage, 0f, Projectile.owner, 0f, 0f);
