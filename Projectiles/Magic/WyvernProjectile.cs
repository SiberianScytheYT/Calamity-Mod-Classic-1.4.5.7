...
namespace CalRD.Projectiles.Magic
{
    public class WyvernProjectile : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            {
                int head = Gore.NewGore(Entity.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 0.8f, Mod.Find<ModGore>("WyvernHead").Type);
                Main.gore[head].timeLeft /= 10;
                int body = Gore.NewGore(Entity.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 0.8f, Mod.Find<ModGore>("WyvernBody").Type);
                Main.gore[body].timeLeft /= 10;
                int tail = Gore.NewGore(Entity.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 0.8f, Mod.Find<ModGore>("WyvernTail").Type);
