        ...
        public override void AI()
        {
			if (Projectile.timeLeft <= 600)
			{
				if (Projectile.ai[0] == 0f && Projectile.owner == Main.myPlayer)
				{
					...
					SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/LightningStrike"), new Vector2((int)Projectile.Center.X, (int)Projectile.Center.Y));
					...
					int proj = Projectile.NewProjectile(Entity.GetSource_FromThis(), fireFrom.X, fireFrom.Y, velocity.X, velocity.Y, ProjectileID.CultistBossLightningOrbArc, damage, 0f, Projectile.owner, ai0.ToRotation(), ai);
