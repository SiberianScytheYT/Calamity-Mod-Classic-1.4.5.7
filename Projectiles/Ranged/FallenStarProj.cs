...
namespace CalRD.Projectiles.Ranged
{
	public class FallenStarProj : ModProjectile
	{
		...
		public override void OnKill(int timeLeft)
		{
			...
			{
				Gore.NewGore(Entity.GetSource_FromThis(), Projectile.position, Projectile.velocity * 0.05f, Main.rand.Next(16, 18), 1f);
			}
		}
        ...
        public override bool PreDraw(ref Color lightColor)
        {
			...
			Texture2D aura = ModContent.Request<Texture2D>("CalRD/Projectiles/Ranged/FallenStarAura").Value;
			...
			//Draw the aura
			Main.spriteBatch.Draw(aura, drawStart + offsets + spinPoint.RotatedBy(MathHelper.TwoPi * time), auraRec, blue, Projectile.velocity.ToRotation() + MathHelper.PiOver2, auraOrigin, 1.5f, SpriteEffects.None, 0);
			Main.spriteBatch.Draw(aura, drawStart + offsets + spinPoint.RotatedBy(MathHelper.TwoPi * time + MathHelper.TwoPi / 3f), auraRec, blue, Projectile.velocity.ToRotation() + MathHelper.PiOver2, auraOrigin, 1.1f, SpriteEffects.None, 0);
			Main.spriteBatch.Draw(aura, drawStart + offsets + spinPoint.RotatedBy(MathHelper.TwoPi * time + MathHelper.Pi * 4f / 3f), auraRec, blue, Projectile.velocity.ToRotation() + MathHelper.PiOver2, auraOrigin, 1.3f, SpriteEffects.None, 0);
			for (float d = 0f; d < 1f; d += 0.5f)
			{
				...
				Main.spriteBatch.Draw(aura, drawStart2 + offsets, auraRec, white * colorMult, Projectile.velocity.ToRotation() + MathHelper.PiOver2, auraOrigin, 0.3f + scaleMult * 0.5f, SpriteEffects.None, 0);
			}

			//Draw the actual projectile
			Main.spriteBatch.Draw(tex, Projectile.Center + offsets, spriteRec, alpha, Projectile.rotation, spriteOrigin, Projectile.scale + 0.1f, spriteEffects, 0);
            return false;
        }
	}
}