        ...
        public override void AI()
        {
            ...
            if (Projectile.localAI[0] > 30f) //dragon dust trail counter, but only empowered proj spawns it
            {
                ...
                if (Projectile.ai[0] != 0f && Projectile.owner == Main.myPlayer)
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<DragonDust>(), Projectile.damage / 3, Projectile.knockBack * 3f, Projectile.owner);
            }
        ...
        public override void OnKill(int timeLeft)
        {
            if (timeLeft != 0)
            {
                ...
                if (Projectile.ai[0] != 0f && Projectile.owner == Main.myPlayer) //if empowered, make exo arrow and dragon dust
                {
                    ...
                    speed /= 30f;
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), vector3.X, vector3.Y, speed.X, speed.Y, ModContent.ProjectileType<DrataliornusExoArrow>(), Projectile.damage / 2, Projectile.knockBack, Projectile.owner);

                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<DragonDust>(), Projectile.damage / 3, Projectile.knockBack * 2f, Projectile.owner);
                }
        ...
        /*
        {
            ...
            if (Projectile.ai[0] != 0f && Projectile.owner == Main.myPlayer) //if empowered
            {
                if (Projectile.timeLeft != 0) //will not be called on npcs hit by explosion (only direct hits)
                {
                    ...
                    vel.Normalize();
                    vel *= 30f;
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), pos, vel + target.velocity, ModContent.ProjectileType<SkyFlareFriendly>(), Projectile.damage * 3, Projectile.knockBack * 5f, Projectile.owner);
                }
            }
        }
        */
