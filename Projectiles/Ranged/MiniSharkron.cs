...
namespace CalRD.Projectiles.Ranged
{
	public class MiniSharkron : ModProjectile
	{
		...
		public override void OnKill(int timeLeft)
		{
			...
			int tail = Gore.NewGore(Entity.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 0.8f, 584, 1f);
			Main.gore[tail].timeLeft /= 10;
			int body = Gore.NewGore(Entity.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 0.9f, 585, 1f);
			Main.gore[body].timeLeft /= 10;
			int head = Gore.NewGore(Entity.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 1f, 586, 1f);
