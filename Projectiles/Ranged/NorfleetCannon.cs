...
namespace CalRD.Projectiles.Ranged
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/Ranged/Norfleet";
            ...
            if (timeToFire && Main.myPlayer == Projectile.owner)
            {
                if (canFire)
                {
                    ...
                    for (int i = 0; i < 5; i++) 
                    {
                        ...
                        Projectile.NewProjectile(Entity.GetSource_FromThis(), position, speed, type, damage, Projectile.knockBack, Projectile.owner);
                        speed.X += Main.rand.NextFloat(-1.5f, 1.5f);
                        speed.Y += Main.rand.NextFloat(-1.5f, 1.5f);
                        Projectile.NewProjectile(Entity.GetSource_FromThis(), position, speed, type, damage, Projectile.knockBack, Projectile.owner);
