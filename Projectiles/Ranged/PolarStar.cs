...
namespace CalRD.Projectiles.Ranged
{
    public class PolarStar : ModProjectile
    {
        ...
        {
            SoundEngine.PlaySound(SoundID.Item62.WithVolumeScale(0.5f), Projectile.position);
            if (Projectile.ai[1] == 1f) //Boost two
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
                    ...
                    {
                        int split = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.position.X, Projectile.position.Y, (float)Main.rand.Next(-10, 11) * 2f, (float)Main.rand.Next(-10, 11) * 2f, ModContent.ProjectileType<ChargedBlast2>(),
