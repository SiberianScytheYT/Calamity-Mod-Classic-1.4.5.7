...
namespace CalRD.Projectiles.Ranged
{
    public class MagnomalyRocket : ModProjectile
    {
        ...
        public override void AI()
        {
        	...
        	{
            	Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<MagnomalyAura>(), (int)(Projectile.damage * 0.5f), Projectile.knockBack * 0.5f, Projectile.owner, Projectile.identity, 0f);
				spawnedAura = true;
			}
        ...
        public override void OnKill(int timeLeft)
        {
			if (Projectile.owner == Main.myPlayer)
			{
				...
				Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<MagnomalyExplosion>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
				...
				CalamityUtils.ExplosionGores(Projectile.GetSource_FromThis(), Projectile.Center, 9);
			}
        }
        ...
        private void OnHitEffects()
        {
			if (Projectile.owner == Main.myPlayer)
			{
				...
				for (int i = 0; i < 4; i++)
				{
					...
					int proj1 = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)(Math.Sin(offsetAngle) * 5f), (float)(Math.Cos(offsetAngle) * 5f), ModContent.ProjectileType<MagnomalyBeam>(), Projectile.damage / 4, Projectile.knockBack / 4, Projectile.owner, 0f, 1f);
					int proj2 = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)(-Math.Sin(offsetAngle) * 5f), (float)(-Math.Cos(offsetAngle) * 5f), ModContent.ProjectileType<MagnomalyBeam>(), Projectile.damage / 4, Projectile.knockBack / 4, Projectile.owner, 0f, 1f);
				}
			}
        }
    }
