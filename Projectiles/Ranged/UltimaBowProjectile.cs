...
namespace CalRD.Projectiles.Ranged
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/Ranged/Ultima";
        ...
        public void AttemptToFireProjectiles(Player player)
        {
            ...
            if (Projectile.owner == Main.myPlayer && Time % player.ActiveItem().useTime == 0)
            {
                ...
                if (Time >= Ultima.FullChargeTime * 0.7f && Main.rand.NextBool(6))
                {
                    ...
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), shotPosition, sparkVelocity, ModContent.ProjectileType<UltimaSpark>(), damage / 3, Projectile.knockBack, Projectile.owner);
                }
                knockBack = player.GetWeaponKnockback(player.ActiveItem(), Projectile.knockBack);

                Projectile.NewProjectile(Entity.GetSource_FromThis(), shotPosition, shotVelocity, type, damage, Projectile.knockBack, Projectile.owner);
