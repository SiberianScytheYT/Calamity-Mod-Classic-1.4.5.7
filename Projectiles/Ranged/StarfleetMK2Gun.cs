...
namespace CalRD.Projectiles.Ranged
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/Ranged/StarfleetMK2";
            ...
            if (timeToFire && Main.myPlayer == Projectile.owner)
            {
                if (canFire)
                {
                    ...
                    for (int i = 0; i < 5; i++)
                    {
                        ...
                        int star = Projectile.NewProjectile(Entity.GetSource_FromThis(), position, speed, type, damage, Projectile.knockBack, Projectile.owner);
