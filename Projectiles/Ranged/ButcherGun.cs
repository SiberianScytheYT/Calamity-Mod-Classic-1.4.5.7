...
namespace CalRD.Projectiles.Ranged
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/Ranged/Butcher";
            ...
            if (willShoot && Main.myPlayer == Projectile.owner)
            {
                ...
                if (canShoot)
                {
                    ...
                    for (int projIndex = 0; projIndex < randomBulletCount; projIndex++)
                    {
                        ...
                        int num44 = Projectile.NewProjectile(Entity.GetSource_FromThis(), source, bulletVel, projType, damage, kback, Projectile.owner, 0f, 0f);
