...
namespace CalRD.Projectiles.Ranged
{
    public class DrizzlefishFire : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            {
                Texture2D texture = ModContent.Request<Texture2D>("CalRD/Projectiles/Ranged/DrizzlefishFire2").Value;
                ...
                if (Projectile.owner == Main.myPlayer)
                {
					if (Projectile.ai[1] == 1f)
					{
						for (int i = 0; i < numProj + 1; i++)
						{
							...
							Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, perturbedSpeed.X, perturbedSpeed.Y, ModContent.ProjectileType<DrizzlefishFireSplit>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 1f);
						}
					}
					else
					{
						...
						{
							Vector2 perturbedSpeed = new Vector2(Projectile.velocity.X, Projectile.velocity.Y).RotatedBy(MathHelper.Lerp(-rotation, rotation, i / (numProj - 1)));
							Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, perturbedSpeed.X, perturbedSpeed.Y, ModContent.ProjectileType<DrizzlefishFireSplit>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
						}
					}
                }
                Projectile.Kill();
