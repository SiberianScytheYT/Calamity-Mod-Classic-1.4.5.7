	...
	public class SepticSkewerHarpoon : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if ((int)Projectile.ai[1] % 4 == 0 && Projectile.owner == Main.myPlayer && Main.rand.NextBool(5))
            {
                ...
                Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, vector63.X, vector63.Y, ModContent.ProjectileType<SepticSkewerBacteria>(), (int)((double)Projectile.damage * 0.4), Projectile.knockBack * 0.2f, Projectile.owner, -10f, 0f);
            }
            if (player.dead)
            {
                Projectile.Kill();
