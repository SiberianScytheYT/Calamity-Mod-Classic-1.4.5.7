...
namespace CalRD.Projectiles.Damageable
{
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 1; i <= 6; i++)
            {
                if (Main.myPlayer == Projectile.owner)
                {
                    ...
                    Projectile shard = Projectile.NewProjectileDirect(Entity.GetSource_FromThis(), Projectile.Center, Vector2.Zero, Mod.Find<ModProjectile>("ArtifactOfResilienceShard" + i.ToString()).Type, Projectile.damage, Projectile.knockBack, Projectile.owner);
                    shard.ai[0] = angle;
