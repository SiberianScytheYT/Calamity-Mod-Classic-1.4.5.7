				...
				if (mode == 0)
				{
					if (targetFound && Projectile.ai[1] == 0f)
					{
						...
						if (Main.myPlayer == Projectile.owner && Collision.CanHitLine(Projectile.position, Projectile.width, Projectile.height, targetLocation, 0, 0))
						{
							...
							Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, projVect.X, projVect.Y, projType, (int)(Projectile.damage * 0.6f), 0f, Main.myPlayer, 0f, 0f);
							Projectile.netUpdate = true;
						}
					}
				}
					...
					if (targetFound && Projectile.ai[1] == 0f)
					{
						...
						if (Main.myPlayer == Projectile.owner && Collision.CanHitLine(Projectile.position, Projectile.width, Projectile.height, targetLocation, 0, 0))
						{
							for (int beeIndex = 0; beeIndex < (projType == bigBee ? 1 : Main.rand.Next(1,5)); beeIndex++)
							{
								...
								int bee = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, SpeedX, SpeedY, projType, (int)(Projectile.damage * 0.8f), 0f, Main.myPlayer, 0f, 0f);
								if (projType == bigBee)
								{
