            ...
            if (Projectile.owner == Main.myPlayer)
            {
                ...
                if (foundTarget && targetIndex != -1)
                {
					...
					if (reversedGravity ? (Main.npc[targetIndex].Bottom.Y > Projectile.Top.Y) : (Main.npc[targetIndex].Bottom.Y < Projectile.Top.Y))
					{
						...
						Projectile.NewProjectile(Entity.GetSource_FromThis(), source, velocity, projectileType, Projectile.damage, 5f, Projectile.owner, 0f, 0f);
					...
					else
					{
						...
						for (int i = 0; i < amount; i++)
						{
							...
							if (reversedGravity)
								velocityY *= -1f;
							int flame = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.oldPosition.X + (Projectile.width / 2), Projectile.oldPosition.Y + (Projectile.height / 2), velocityX, velocityY, projectileType, Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
				...
				{
					int smoke = Gore.NewGore(Entity.GetSource_FromThis(), goreVec, default, Main.rand.Next(375, 378), 0.5f);
