    ...
    public class MagicUmbrella : ModProjectile
    {
		...
		private void SpawnBaseballBats(Vector2 targetPos)
		{
            for (int n = 0; n < Main.rand.Next(1, 3); n++) //1 to 2 baseball bats
            {
                ...
                Projectile.NewProjectile(Entity.GetSource_FromThis(), source, velocity, ModContent.ProjectileType<MagicBat>(), (int)(Projectile.damage * Main.rand.NextFloat(0.3f, 0.6f)), Projectile.knockBack * Main.rand.NextFloat(0.7f, 1f), Projectile.owner, 0f, 0f);
            }
		}
    }
}
