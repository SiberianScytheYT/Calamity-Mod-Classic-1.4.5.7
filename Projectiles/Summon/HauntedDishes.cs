...
namespace CalRD.Projectiles.Summon
{
	public class HauntedDishes : ModProjectile
    {
				...
				else
				{
					...
					else if (num9 >= 0 && num7 < 800f + conflict3)
					{
						...
						else if (Projectile.owner == Main.myPlayer)
						{
							...
							int index = Projectile.NewProjectile(Entity.GetSource_FromThis(), vector2.X, vector2.Y, SpeedX * 2f, SpeedY * 2f, Type, damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
							if (SpeedX < 0f)
        ...
        {
			int index = Gore.NewGore(Entity.GetSource_FromThis(), new Vector2(Projectile.position.X - (float) (Projectile.width / 2), Projectile.position.Y - (float) (Projectile.height / 2)), new Vector2(0.0f, 0.0f), Main.rand.Next(61, 64), Projectile.scale);
