            ...
            if (circling)
            {
                ...
                if (circlingPlayer)
                {
                    ...
                    if (target != null && Projectile.owner == Main.myPlayer)
                    {
                        ...
                        velocity.Normalize();
                        velocity *= 20f;
                        Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.position, velocity, Projectile.type, (int)(Projectile.damage * 1.05f), Projectile.knockBack, Projectile.owner, Projectile.ai[0], 1f);
