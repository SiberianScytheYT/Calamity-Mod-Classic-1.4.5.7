...
namespace CalRD.Projectiles.Summon
{
	public class VileFeederSummon : ModProjectile
	{
				...
				if (Projectile.owner == Main.myPlayer)
				{
					...
					if (eaterCooldown <= 0)
					{
						...
						for (int index2 = 0; index2 < projNumber; index2++)
						{
							...
							xVector *= 10f;
							yVector *= 10f;
							Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, xVector, yVector, ModContent.ProjectileType<VileFeederProjectile>(), (int)(Projectile.damage * 1.25f), Projectile.knockBack, Projectile.owner, 0f, 0f);
						}
						eaterCooldown = 80;
					}
				}
		...
		public override bool? CanDamage()/* tModPorter Suggestion: Return null instead of true */ => Projectile.ai[0] != 3f;
