    ...
    public class MechwormHead : ModProjectile
    {
            ...
            if (AttackStateTimer % (LaserChargeFrames + LaserRedirectFrames) == LaserChargeFrames)
            {
                ...
                if (Main.myPlayer == Projectile.owner)
                {
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, perturbedSpeed, ModContent.ProjectileType<MechwormLaser>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                    }
                }
        ...
        private void PortalAttackMovement(NPC target)
        {
            ...
            if (AttackStateTimer % chargeTime == 0)
            {
                ...
                if (Main.myPlayer == Projectile.owner)
                {
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), TeleportStartingPoint, Vector2.Zero, ModContent.ProjectileType<MechwormTeleportRift>(), 0, 0f, Projectile.owner);
                    int endGateIndex = Projectile.NewProjectile(Entity.GetSource_FromThis(), TeleportEndingPoint, Vector2.Zero, ModContent.ProjectileType<MechwormTeleportRift>(), 0, 0f, Projectile.owner);
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            Main.spriteBatch.Draw(ModContent.Request<Texture2D>("CalRD/Projectiles/Summon/MechwormHeadGlow").Value, Projectile.Center - Main.screenPosition, null, Color.White, Projectile.rotation, origin, 1f, SpriteEffects.None, 0f);
        }
