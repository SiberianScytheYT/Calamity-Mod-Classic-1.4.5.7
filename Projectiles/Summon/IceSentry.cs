            ...
            else
            {
                ...
                if (Projectile.localAI[1] > 2)
                {
                    ...
                    if (Projectile.owner == Main.myPlayer)
                    {
                        ...
                        speed.Normalize();
                        speed *= 15f;
                        int shard = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center + speed, speed, ModContent.ProjectileType<IceSentryShard>(), Projectile.damage / 2, Projectile.knockBack / 2, Projectile.owner);
                    }
                }
            }
                ...
                if (rememberTarget)
                {
                    ...
                    if (Projectile.localAI[0] > delay)
                    {
                        ...
                        if (rememberTarget && Projectile.owner == Main.myPlayer)
                        {
                            ...
                            Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, speed + npc.velocity / 2f, ModContent.ProjectileType<IceSentryFrostBolt>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                        }
                    }
                }
