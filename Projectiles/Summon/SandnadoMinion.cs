                    ...
                    if (!Collision.SolidCollision(Projectile.position, Projectile.width, Projectile.height))
                    {
                        if (Projectile.ai[1] == 0f)
                        {
                            ...
                            if (Main.myPlayer == Projectile.owner)
                            {
                                ...
                                velocity.Normalize();
                                velocity *= speed;
                                int shark = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, velocity, projType, Projectile.damage, Projectile.knockBack, Projectile.owner);
