...
using CalRD.Items;
using CalRD.Projectiles.Summon.AndromedaUI;
...
namespace CalRD.Projectiles.Summon
{
    public class GiantIbanRobotOfDoom : ModProjectile
    {
        ...
        {
            EquipLoader.AddEquipTexture(Mod, "CalRD/ExtraTextures/AndromedaWithout_Head", EquipType.Head, name: "NoHead");
        ...
        public void RegisterRightClick(Player player)
        {
            if (Main.mouseRight && ClickCooldown <= 0f)
            {
                ...
                else
                {
                    ...
                    {
                        Projectile ui = Projectile.NewProjectileDirect(Entity.GetSource_FromThis(), 
                                                Main.MouseWorld,
                                                 Vector2.Zero,
            ...
            if (LaserCooldown % (AndromedaDeathRay.TrueTimeLeft - 10) == (AndromedaDeathRay.TrueTimeLeft - 11))
            {
                if (Projectile.owner == Main.myPlayer)
                {
                    ...
                    if (player.HeldItem != null)
                    {
                        ...
                        else if (player.HeldItem.CountsAsClass(DamageClass.Melee))
                        {
                            damage = (int)(damage * player.MeleeDamage());
                        }
                        else if (player.HeldItem.CountsAsClass(DamageClass.Ranged))
                        {
                    ...
                    Projectile deathLaser = Projectile.NewProjectileDirect(Entity.GetSource_FromThis(), Projectile.Center,
                                                                           laserVelocity,
