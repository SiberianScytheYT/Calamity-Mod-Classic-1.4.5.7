    ...
    public class PlantSummon : ModProjectile
    {
					...
					if (Projectile.ai[1] == 0f && foundTarget && range < 500f)
					{
						...
						if (Main.myPlayer == Projectile.owner)
						{
							...
							else
							{
								...
								velocity *= projSpeed;
								velocity *= speedMult;
								Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, velocity, projType, projDmg, Projectile.knockBack, Projectile.owner, 0f, 0f);
							}
					...
					if (Projectile.ai[1] == 0f && foundTarget && range < 500f)
					{
						...
						if (Main.myPlayer == Projectile.owner)
						{
							...
							if (Main.rand.NextBool(2))
							{
								...
								Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, projVelocity, projType, projDmg, Projectile.knockBack, Projectile.owner, 0f, 1f);
							}
		...
		private void FireShotgun(Vector2 whereIsTarget, float attackMult)
		{
			...
			for (int i = -8; i <= 8; i += 8)
			{
				...
				Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, perturbedSpeed, projType, projDmg, Projectile.knockBack * attackMult, Projectile.owner, Main.rand.Next(3), 1f);
			}
		}
		...
		private void SpawnTentacles()
		{
			if (Projectile.owner == Main.myPlayer)
			{
				...
				{
					Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<PlantTentacle>(), Projectile.damage, Projectile.knockBack, Projectile.owner, tentacleIndex, Projectile.GetByUUID(Projectile.owner, Projectile.whoAmI));
				}
			}
		}
        ...
        public override bool? CanDamage()/* tModPorter Suggestion: Return null instead of true */ => enraged;
