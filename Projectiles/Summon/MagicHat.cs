                ...
                if (enemyDetected)
                {
					...
					if (Projectile.ai[1] % 5f == 0f)
					{
						...
						for (int i = 0; i < amount; i++)
						{
							...
							Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.oldPosition.X + (float)(Projectile.width / 2), Projectile.oldPosition.Y + (float)(Projectile.height / 2), velocityX, velocityY, projType, Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
						}
					}
                }
            }
