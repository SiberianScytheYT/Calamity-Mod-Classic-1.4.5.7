...
namespace CalRD.Projectiles.Summon
{
	public class EndoHydraHead : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (body.ai[0] >= 0 && body.ai[0] < Main.npc.Length)
            {
                ...
                if (targetAliveAndInLineOfSight && target.CanBeChasedBy() && Main.myPlayer == Projectile.owner)
                {
                    if (Time % 40f == 24f)
                    {
                        Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, Projectile.DirectionTo(target.Center) * 6f, ModContent.ProjectileType<EndoRay>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                    }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D chain = ModContent.Request<Texture2D>("CalRD/Projectiles/Summon/EndoHydraChain").Value;
