			...
			if (Projectile.owner == Main.myPlayer && homeIn)
            {
                ...
                if ((Projectile.ai[1] % 10f) == 0f)
                {
                    ...
                    for (int i = 0; i < amount; i++)
                    {
                        ...
                        Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.oldPosition.X + (Projectile.width / 2), Projectile.oldPosition.Y + (Projectile.height / 2), velocityX, velocityY, ModContent.ProjectileType<SpikecragSpike>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                    }
                }
            }
        }
