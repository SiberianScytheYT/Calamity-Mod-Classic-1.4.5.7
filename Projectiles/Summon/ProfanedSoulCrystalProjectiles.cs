...
namespace CalRD.Projectiles.Summon
{
    ...
    public class ProfanedCrystalMageFireball : ModProjectile
    {
        ...
        private void Split(bool hit, bool chaseable)
        {
            ...
            for (int i = 0; i < outerSplits; i++) //use outer since it has the higher number regardless
            {
                ...
                velocity.Normalize();
                velocity *= 8f;
                int proj = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center + outerPosVec, velocity, ModContent.ProjectileType<ProfanedCrystalMageFireballSplit>(), damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                if (proj != -1)
                ...
                if (innerSplits > 0) //only runs if there's still inner splits to create
                {
                    ...
                    velocity.Normalize();
                    velocity *= 5f;
                    proj = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center + innerPosVec, velocity, ModContent.ProjectileType<ProfanedCrystalMageFireballSplit>(), damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                    if (proj != -1)
                        Main.projectile[proj].Calamity().forceMinion = true;
                }
                innerSplits--;
    ...
    public class ProfanedCrystalMageFireballSplit : ModProjectile
    {
        ...
        {
            bool instakill = target.type == ModContent.NPCType<StormWeaverBody>() || target.type == ModContent.NPCType<StormWeaverHead>() || target.type == ModContent.NPCType<SCalWormHead>()
    || target.type == ModContent.NPCType<SCalWormBody>() || target.type == ModContent.NPCType<SCalWormBodyWeak>() || target.type == ModContent.NPCType<SCalWormTail>();
        ...
        private void handleSpecialHit(Vector2 targCenter)
        {
            if (Projectile.owner == Main.myPlayer)
            {
                for (int i = 0; i < 10; ++i)
                {
                    ...
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), startPoint, velocity, ModContent.ProjectileType<ProfanedCrystalMeleeSpear>(), (int)(0.25f * Projectile.damage), 0f, Projectile.owner, 2f, 0f);
                }
            }
        }
        ...
        private void swarm(Vector2 targetPos)
        {
            if (Projectile.owner == Main.myPlayer)
            {
                ...
                for (float i = 0; i < swarmAmount; i++)
                {
                    ...
                    int proj = Projectile.NewProjectile(Entity.GetSource_FromThis(), pos, correctedVelocity, ModContent.ProjectileType<ProfanedCrystalRangedHuges>(), (int)((double)Projectile.damage * 1.5f), Projectile.knockBack, Projectile.owner, 2);
                    Main.projectile[proj].Calamity().forceMinion = true;
    ...
    || target.type == ModContent.NPCType<SCalWormBody>() || target.type == ModContent.NPCType<SCalWormBodyWeak>() || target.type == ModContent.NPCType<SCalWormTail>();
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int num625 = 0; num625 < 3; num625++)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    int num626 = Gore.NewGore(Entity.GetSource_FromThis(), new Vector2(Projectile.position.X + (float)(Projectile.width / 2) - 24f, Projectile.position.Y + (float)(Projectile.height / 2) - 24f), default, Main.rand.Next(61, 64), 1f);
