	...
	public class RustyDrone : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            else
            {
                ...
                if (Projectile.Distance(destination) < 55f)
                {
                    if (Projectile.ai[0]++ % AttackRate == AttackRate - 1)
                    {
                        ...
                        {
                            Projectile.NewProjectile(Entity.GetSource_FromThis(), potentialTarget.Center, Vector2.Zero, ModContent.ProjectileType<RustyDroneTargetIndicator>(), 0, 0f, Projectile.owner, Projectile.whoAmI, potentialTarget.whoAmI);
                        }
                    }
                }
            }
            ...
            if (Projectile.ai[1] >= 360f)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, Vector2.UnitY.RotatedByRandom(MathHelper.TwoPi) * 8f, ModContent.ProjectileType<RustShrapnel>(),
        ...
        public override bool? CanDamage()/* tModPorter Suggestion: Return null instead of true */ => Projectile.localAI[0] == 0f; // Only do damage from the initial 1 damage prick, and never again. This is a support summon.
    }
}
