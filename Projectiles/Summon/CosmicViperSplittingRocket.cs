					...
					if (Vector2.Distance(npc.Center, Projectile.Center) < (explode + extraDistance))
					{
						...
						if (Projectile.owner == Main.myPlayer)
						{
							for (int i = 0; i < numProj; i++)
							{
								...
								Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, speed.X, speed.Y, ModContent.ProjectileType<CosmicViperSplitRocket1>(), (int)(Projectile.damage * 0.25), Projectile.knockBack, Projectile.owner, 0f, 0f);
							}
                        }
						...
						if (Vector2.Distance(npc.Center, Projectile.Center) < (explode + extraDistance))
						{
							...
							if (Projectile.owner == Main.myPlayer)
							{
								for (int i = 0; i < numProj; i++)
								{
									...
									speed.Normalize();
									speed *= (float)Main.rand.Next(30, 61) * 0.1f * 2f;
									Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, speed.X, speed.Y, ModContent.ProjectileType<CosmicViperSplitRocket1>(), (int)(Projectile.damage * 0.25f), Projectile.knockBack, Projectile.owner, Main.rand.Next(2), 0f);
								}
                            }
                            SoundEngine.PlaySound(SoundID.Item14, Projectile.position);
                            Projectile.Kill();
