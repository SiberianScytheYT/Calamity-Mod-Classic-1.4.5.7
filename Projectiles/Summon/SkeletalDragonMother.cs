	...
	public class SkeletalDragonMother : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.localAI[0] == 0f)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center + Utils.RandomVector2(Main.rand, -24f, 24f), 
            ...
            if (target != null)
            {
                ...
                if ((modulo >= 30f && modulo <= 60f) || (modulo >= 90f && modulo <= 120f))
                {
                    ...
                    {
                        Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, Projectile.DirectionTo(target.Center) * 11.5f, ModContent.ProjectileType<BloodBreath>(), Projectile.damage, 0f, Projectile.owner);
                    }
                }
            }
