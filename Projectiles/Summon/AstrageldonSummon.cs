			...
			if (Projectile.frame == 0 || Projectile.frame == 1)
			{
				...
				if (gotoenemy && attackCounter == 0)
				{
					...
					if (Main.myPlayer == Projectile.owner)
					{
						...
						int laser = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, laserVel.X, laserVel.Y, projType, Projectile.damage, 0f, Projectile.owner, 0f, 0f);
						Projectile.netUpdate = true;
					}
				}
			}
