            ...
            if (Projectile.owner == Main.myPlayer)
            {
				for (int l = 0; l < Main.maxNPCs; l++)
				{
					...
					if (npc.active && !npc.friendly && npc.damage > 0 && !npc.dontTakeDamage && !npc.buffImmune[buffType] && Vector2.Distance(Projectile.Center, npc.Center) <= range)
					{
						...
						if (dealDamage)
						{
							Projectile aura = Projectile.NewProjectileDirect(Entity.GetSource_FromThis(), npc.Center, Vector2.Zero, ModContent.ProjectileType<DirectStrike>(), dmg, 0f, Projectile.owner, l);
