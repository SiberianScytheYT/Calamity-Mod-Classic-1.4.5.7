...
namespace CalRD.Projectiles.Hybrid
{
	...
	{
        public override string Texture => "CalRD/Items/Weapons/Melee/PwnagehammerMelee";
		...
		public override void OnKill(int timeLeft)
		{
			...
			if (Projectile.ai[0] == 1f)
			{
				...
				for (int k = 0; k < Main.maxNPCs; k++)
				{
					...
					if (distanceTo < distance && npc.CanBeChasedBy(Projectile, false) && k != Projectile.localAI[0])
					{
						...
						double damage = npc.StrikeNPC(npc.CalculateHitInfo((int)alldamage, (Projectile.velocity.X > 0f) ? 1 : -1, false, Projectile.knockBack));
						player.addDPS((int)damage);
					}
				}
				SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Item/PwnagehammerHoming") with {Volume = 0.3f}, Projectile.Center);
			...
			else
			{
				...
				SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Item/PwnagehammerSound") with {Volume = 0.16f, Pitch = Main.rand.NextFloat(-0.08f, 0.08f)}, Projectile.Center);
