...
namespace CalRD.Projectiles.Hybrid
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/Melee/GalaxySmasherMelee";
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int i = 0; i < numLasers; ++i)
            {
                ...
                {
                    int proj = Projectile.NewProjectile(Entity.GetSource_FromThis(), startPoint, velocity, laserID, laserDamage, laserKB, Projectile.owner);
        ...
        public override void OnHitPlayer(Player target, Player.HurtInfo info)
        {
            ...
            for (int i = 0; i < numLasers; ++i)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
                    float damageType = Projectile.CountsAsClass(DamageClass.Melee) ? 1f : 2f;
                    int proj = Projectile.NewProjectile(Entity.GetSource_FromThis(), startPoint, velocity, laserID, laserDamage, laserKB, Projectile.owner, damageType, 0f);
                    Main.projectile[proj].tileCollide = false;
                    Main.projectile[proj].timeLeft = 30;
                }
            }
