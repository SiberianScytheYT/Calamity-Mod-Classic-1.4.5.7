...
namespace CalRD.Projectiles.Hybrid
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/Rogue/TerraDisk";
                    ...
                    if (Main.rand.NextBool(5))
                    {
                        if (Projectile.owner == Main.myPlayer)
                        {
                            ...
                            for (i = 0; i < 3; i++)
                            {
                                ...
                                int proj = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)(Math.Sin(offsetAngle) * 5f), (float)(Math.Cos(offsetAngle) * 5f), ModContent.ProjectileType<TerraDiskProjectile2>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
								if (Projectile.CountsAsClass(DamageClass.Melee))
									Main.projectile[proj].Calamity().forceMelee = true;
								else
									Main.projectile[proj].Calamity().forceRogue = true;
                                int proj2 = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)(-Math.Sin(offsetAngle) * 5f), (float)(-Math.Cos(offsetAngle) * 5f), ModContent.ProjectileType<TerraDiskProjectile2>(), Projectile.damage / 2, Projectile.knockBack, Projectile.owner, 0f, 0f);
								if (Projectile.CountsAsClass(DamageClass.Melee))
