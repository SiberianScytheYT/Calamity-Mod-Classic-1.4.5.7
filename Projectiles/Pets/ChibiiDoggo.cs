...
namespace CalRD.Projectiles.Pets
{
    public class ChibiiDoggo : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D texture2D3 = ModContent.Request<Texture2D>("CalRD/Projectiles/Pets/ChibiiDoggoMonochrome").Value;
        ...
        public void SpawnDoggo()
        {
            ...
            int n = NPC.NewNPC(Entity.GetSource_FromThis(), (int)Projectile.position.X, (int)Projectile.position.Y, type);
        ...
        public override void AI()
        {
            ...
            else
            {
                ...
                if (player.ownedProjectileCounts[ModContent.ProjectileType<ChibiiDoggoFly>()] <= 0)
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.position.X, Projectile.position.Y, 0f, 0f, ModContent.ProjectileType<ChibiiDoggoFly>(), 0, 0f, Projectile.owner, (float)Projectile.identity);
            }
            ...
            if ((double)notlocalai1 > (double)Main.rand.Next(30, 120) && !player.immune && player.velocity.X == 0 && player.velocity.Y == 0)
            {
                if (Main.rand.NextBool(3))
                {
                    if (Main.rand.NextBool(2))
                    {
                        ...
                        SoundEngine.PlaySound(SoundID.Meowmere, Projectile.position); //nya
                    }
                    else
                    {
