...
namespace CalRD.Projectiles.Melee.Spears
{
    public class ExsanguinationLanceProjectile : BaseSpearProjectile
    {
        ...
        public override void ExtraBehavior()
        {
			...
			{
				int smoke = Gore.NewGore(Entity.GetSource_FromThis(), goreVec, default, Main.rand.Next(375, 378), 1f);
		...
		private void OnHitEffects(Vector2 targetPos, bool crit)
		{
            ...
            if (crit)
            {
                for (int i = 0; i < 2; i++)
                {
					if (Projectile.owner == Main.myPlayer)
					{
						CalamityUtils.ProjectileBarrage(Projectile.GetSource_FromThis(), Projectile.Center, targetPos, Main.rand.NextBool(), 800f, 800f, 0f, 800f, 10f, ModContent.ProjectileType<TinyFlare>(), (int)(Projectile.damage * 0.5), 2f, Projectile.owner, true);
					}
				}
			}
        }
