...
namespace CalRD.Projectiles.Melee.Spears
{
    public class BansheeHookProj : BaseSpearProjectile
    {
        ...
        {
            Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center + Projectile.velocity * 0.5f,
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D alternateHookTexture = Projectile.spriteDirection == -1 ? ModContent.Request<Texture2D>("CalRD/Projectiles/Melee/Spears/BansheeHookAlt").Value : TextureAssets.Projectile[Projectile.type].Value;
            Vector2 origin = new Vector2(Projectile.spriteDirection == 1 ? alternateHookTexture.Width + 8f : -8f, -8f);
            Main.spriteBatch.Draw(alternateHookTexture, drawPosition, null, 
		...
		{
			Vector2 drawPosition = Projectile.position + new Vector2(Projectile.width, Projectile.height) / 2f + Vector2.UnitY * Projectile.gfxOffY - Main.screenPosition;
			Texture2D texture = Projectile.spriteDirection == -1 ? ModContent.Request<Texture2D>("CalRD/Projectiles/Melee/Spears/BansheeHookAltGlow").Value : ModContent.Request<Texture2D>("CalRD/Projectiles/Melee/Spears/BansheeHookGlow").Value;
			Vector2 origin = new Vector2(Projectile.spriteDirection == 1 ? texture.Width - -8f : -8f, -8f); //-8 -8
			Main.spriteBatch.Draw(texture, drawPosition, null, Color.White, Projectile.rotation, origin, 1f, SpriteEffects.None, 0f);
		}
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            {
                Projectile.NewProjectile(Entity.GetSource_FromThis(), target.Center, Vector2.Zero,
                    ModContent.ProjectileType<BansheeHookBoom>(), (int)(Projectile.damage * 0.25),
