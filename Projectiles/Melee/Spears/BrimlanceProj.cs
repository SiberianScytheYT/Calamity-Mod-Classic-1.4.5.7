...
namespace CalRD.Projectiles.Melee.Spears
{
    public class BrimlanceProj : BaseSpearProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (target.life <= 0)
            {
                int flameCount = Main.rand.Next(2, 5);
                if (Projectile.owner == Main.myPlayer)
                {
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), target.Center, Vector2.Zero, ModContent.ProjectileType<BrimlanceHellfireExplosion>(), (int)(Projectile.damage * 0.75), Projectile.knockBack, Main.myPlayer);
                    for (int i = 0; i < flameCount; i++)
                    {
                        Vector2 fireVelocity = new Vector2(0f, Main.rand.NextFloat(7f, 10f)).RotatedByRandom(MathHelper.TwoPi);
                        Projectile.NewProjectile(Entity.GetSource_FromThis(), target.Center, fireVelocity, ModContent.ProjectileType<BrimlanceStandingFire>(), (int)(Projectile.damage * 0.5), 0f, Projectile.owner, 0f, 0f);
                    }
                }
            }
        }
