    ...
    public class OracleYoyo : ModProjectile
    {
        ...
        private void DealAuraDamage(float radius, int baseDamage)
        {
            ...
            for (int i = 0; i < Main.maxNPCs; ++i)
            {
                ...
                if (dist <= radius)
                {
                    ...
                    if (Projectile.owner == Main.myPlayer)
                    {
                        Projectile p = Projectile.NewProjectileDirect(Entity.GetSource_FromThis(), target.Center, Vector2.Zero, ModContent.ProjectileType<DirectStrike>(), finalDamage, 0f, Projectile.owner, i);
        ...
        private void FireAuricOrbs()
        {
            ...
            for (int i = 0; i < numOrbs; ++i)
            {
                ...
                velocity *= 18f;
                if (Projectile.owner == Main.myPlayer)
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center + posVec, velocity, orbID, orbDamage, orbKB, Main.myPlayer, 0.0f, 0.0f);
            }
        }
    }
}
