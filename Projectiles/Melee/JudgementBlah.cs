...
namespace CalRD.Projectiles.Melee
{
    public class JudgementBlah : ModProjectile
    {
            ...
            if (whiteLightTimer == 0)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
                    for (i = 0; i < 1; i++)
                    {
                        ...
                        int projectile1 = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)(Math.Sin(offsetAngle) * 6f), (float)(Math.Cos(offsetAngle) * 6f), ModContent.ProjectileType<WhiteOrbBlah>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                        int projectile2 = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)(-Math.Sin(offsetAngle) * 6f), (float)(-Math.Cos(offsetAngle) * 6f), ModContent.ProjectileType<WhiteOrbBlah>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
		...
		public override void OnKill(int timeLeft)
        {
			...
			if (Projectile.owner == Main.myPlayer)
            {
                Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y - 100, 0f, 0f, ModContent.ProjectileType<WhiteBoltAuraBlah>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
            }
        }
    }
}
