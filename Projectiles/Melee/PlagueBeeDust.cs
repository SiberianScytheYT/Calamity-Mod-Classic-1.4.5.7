...
namespace CalRD.Projectiles.Melee
{
	...
	{
        public override string Texture => "CalRD/Projectiles/Melee/PlagueDust";
			...
			if (Projectile.ai[1] < 60f)
			{
				if (Projectile.ai[0] > 7f)
				{
					...
					if (Projectile.ai[0] % 2f == 0f)
					{
						...
						int bee = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X + (float)Main.rand.Next(-spawnX, spawnX), Projectile.Center.Y + (float)Main.rand.Next(-spawnY, spawnY), Projectile.velocity.X, Projectile.velocity.Y, player.beeType(), player.beeDamage(Projectile.damage / 3), player.beeKB(0f), Projectile.owner, 0f, 0f);
