    ...
    public class CometQuasherMeteor : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
			...
			if (Projectile.Calamity().lineColor == 1)
				tex = ModContent.Request<Texture2D>("CalRD/Projectiles/Melee/CometQuasherMeteor2").Value;
			if (Projectile.Calamity().lineColor == 2)
				tex = ModContent.Request<Texture2D>("CalRD/Projectiles/Melee/CometQuasherMeteor3").Value;
        ...
        public override void OnKill(int timeLeft)
        {
			...
			{
				int index2 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, DustID.Torch, 0.0f, 0.0f, 100, new Color(), 2.5f);
			...
			{
				int index2 = Gore.NewGore(Entity.GetSource_FromThis(), Projectile.position + new Vector2((float) (Projectile.width * Main.rand.Next(100)) / 100f, (float) (Projectile.height * Main.rand.Next(100)) / 100f) - Vector2.One * 10f, new Vector2(), Main.rand.Next(61, 64), 1f);
			...
			{
				int index2 = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, Utils.SelectRandom<int>(Main.rand, new int[3]{ DustID.Torch, 259, 158 }), 2.5f * (float) Projectile.direction, -2.5f, 0, new Color(), 1f);
				Dust dust1 = Main.dust[index2];
				dust1.alpha = 200;
				dust1.velocity *= 2.4f;
