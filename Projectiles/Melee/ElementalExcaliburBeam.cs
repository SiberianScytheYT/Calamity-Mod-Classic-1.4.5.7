                    ...
                    if (Projectile.localAI[1] >= 180f)
                    {
                        ...
                        if (Projectile.owner == Main.myPlayer)
                        {
                            for (int i = 0; i < numProj + 1; i++)
                            {
                                ...
                                Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, perturbedSpeed * 0.5f, ModContent.ProjectileType<ElementalExcaliburBeam>(), (int)(Projectile.damage * 0.5), Projectile.knockBack * 0.5f, Projectile.owner, 0f, 0f);
                            }
                        }
                        Projectile.Kill();
                    }
                ...
                case 11: // Hot Pink, split into fuschia while travelling
                    ...
                    if (Projectile.localAI[1] >= 181f)
                    {
                        ...
                        if (Main.myPlayer == Projectile.owner)
                            Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center + Projectile.velocity, Projectile.velocity, ModContent.ProjectileType<ElementalExcaliburBeam>(), Projectile.damage / 2, Projectile.knockBack * 0.5f, Projectile.owner, 10f, 0f);
                    }
            ...
            switch ((int)Projectile.ai[0])
            {
                ...
                case 6: // Cyan
                    ...
                    for (int x = 0; x < 3; x++)
                    {
						...
						if (Projectile.owner == Main.myPlayer)
						{
							CalamityUtils.ProjectileBarrage(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.Center, fromRight, 500f, 500f, 0f, 500f, 5f, ModContent.ProjectileType<ElementalExcaliburBeam>(), (int)(Projectile.damage * 0.2), Projectile.knockBack * 0.2f, Projectile.owner, false, 0f).ai[0] = 5f;
						}
                    }
