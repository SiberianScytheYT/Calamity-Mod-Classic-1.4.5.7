    ...
    public class DragonPowFlail : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[1] % 4 == 0)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, vel, type, damage, kb, Projectile.owner, sparkVariety, 0f);
            }
        }
        ...
        private void PetalStorm(Vector2 targetPos)
        {
            ...
            for (int i = 0; i < numPetals; ++i)
            {
				if (Projectile.owner == Main.myPlayer)
				{
					...
					Projectile petal = CalamityUtils.ProjectileBarrage(Projectile.GetSource_FromThis(), Projectile.Center, targetPos, Main.rand.NextBool(), 1000f, 1400f, 80f, 900f, Main.rand.NextFloat(DragonPow.MinPetalSpeed, DragonPow.MaxPetalSpeed), type, petalDamage, petalKB, Projectile.owner);
        ...
        private void Waterfalls(Vector2 targetPos)
        {
            ...
            for (int i = 0; i < numWaterfalls; ++i)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
                    int idx = Projectile.NewProjectile(Entity.GetSource_FromThis(), startPos, fallVec, type, waterfallDamage, waterfallKB, Projectile.owner, 0f, 0f);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D chainTex = ModContent.Request<Texture2D>("CalRD/ExtraTextures/Chains/DragonPowChain").Value;
