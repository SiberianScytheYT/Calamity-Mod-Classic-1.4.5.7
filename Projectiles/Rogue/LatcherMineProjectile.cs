        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < (Projectile.Calamity().stealthStrike ? 7 : 4); i++)
            {
                if (Main.rand.NextBool(2) && Projectile.Calamity().stealthStrike)
                {
                    ...
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Top, shrapnelVelocity, ModContent.ProjectileType<BarrelShrapnel>(), Projectile.damage, 3f, Projectile.owner);
                }
                else
                {
                    Vector2 fireVelocity = (Vector2.UnitY * (-16f + Main.rand.NextFloat(-3, 12f))).RotatedByRandom((double)MathHelper.ToRadians(40f));
                    int fireIndex = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Top, fireVelocity, ModContent.ProjectileType<TotalityFire>(), Projectile.damage / 3, 1f, Projectile.owner);
