...
namespace CalRD.Projectiles.Rogue
{
	public class FinalDawnProjectile : ModProjectile
	{
		...
		public void AttemptExecuteAttacks(Player player)
		{
			...
			if (Projectile.ai[0] >= MaxChargeTime && !player.noItems && !player.CCed)
			{
				...
				if (player.controlUp)
				{
					...
					{
						int stealth = Projectile.NewProjectile(Entity.GetSource_FromThis(), player.Center,
				...
				else
				{
					...
					if (player.Calamity().StealthStrikeAvailable() && Projectile.ai[1] != 1f)
					{
						int stealth = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center,
												 Projectile.velocity,
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			Texture2D glowmask = ModContent.Request<Texture2D>("CalRD/Projectiles/Rogue/FinalDawnProjectile_Glow").Value;
