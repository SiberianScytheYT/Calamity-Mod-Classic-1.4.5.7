...
namespace CalRD.Projectiles.Rogue
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/Rogue/SandDollar";
		...
		private void OnHitEffects()
		{
			...
			if (Projectile.owner == Main.myPlayer && Projectile.Calamity().stealthStrike)
			{
				for (int coralCount = 0; coralCount < coralAmt; coralCount++)
				{
					...
					int coral = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, velocity, ModContent.ProjectileType<SmallCoral>(), Projectile.damage / 3, 0f, Projectile.owner, 0f, 0f);
