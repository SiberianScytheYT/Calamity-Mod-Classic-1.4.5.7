...
namespace CalRD.Projectiles.Rogue
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/Rogue/RadiantStar";
            ...
            if (Projectile.ai[1] == 25f)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
                    for (int i = 0; i < numProj; i++)
                    {
                        ...
                        int stabber2 = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, speed.X, speed.Y, ModContent.ProjectileType<RadiantStar2>(), (int)(Projectile.damage * 0.5f), Projectile.knockBack, Projectile.owner,
                            Projectile.ai[0] == 1f ? 1f : 0f, 0f);
                        Main.projectile[stabber2].Calamity().stealthStrike = Projectile.Calamity().stealthStrike;
                    }
                    SoundEngine.PlaySound(SoundID.Item14, Projectile.position);
                    int boomer = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, 0f, 0f, ModContent.ProjectileType<RadiantExplosion>(), (int)(Projectile.damage * 0.75f), Projectile.knockBack, Projectile.owner, 0f, 0f);
