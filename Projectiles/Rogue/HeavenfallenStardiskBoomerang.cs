...
namespace CalRD.Projectiles.Rogue
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/Rogue/HeavenfallenStardisk";
			...
			if (Projectile.Calamity().stealthStrike)
			{
				if (Projectile.timeLeft % 5f == 0f) //every 5 ticks
				{
					if (Main.rand.NextBool(2))
					{
						...
						{
							CalamityUtils.ProjectileRain(Projectile.GetSource_FromThis(), Projectile.Center, 400f, 100f, 500f, 800f, 29f, ModContent.ProjectileType<HeavenfallenEnergy>(), (int)(Projectile.damage * 0.4), Projectile.knockBack * 0.4f, Projectile.owner);
						}
					}
				}
			}
            ...
            if (explode && player.position.Y != player.oldPosition.Y)
            {
                if (Projectile.owner == Main.myPlayer)
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)(Math.Sin(offsetAngle) * 2f), (float)(Math.Cos(offsetAngle) * 2f), ModContent.ProjectileType<HeavenfallenEnergy>(), (int)(Projectile.damage * 0.4), Projectile.knockBack * 0.4f, Projectile.owner, 0f, 0f);
                        Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)(-Math.Sin(offsetAngle) * 2f), (float)(-Math.Cos(offsetAngle) * 2f), ModContent.ProjectileType<HeavenfallenEnergy>(), (int)(Projectile.damage * 0.4), Projectile.knockBack * 0.4f, Projectile.owner, 0f, 0f);
                    }
                }
            }
