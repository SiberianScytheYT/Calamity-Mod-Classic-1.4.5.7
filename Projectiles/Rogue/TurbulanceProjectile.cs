...
namespace CalRD.Projectiles.Rogue
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/Rogue/Turbulance";
        ...
        public override void AI()
        {
            ...
            if (Projectile.Calamity().stealthStrike) //Stealth strike
			{
				...
				{
					Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<TurbulanceWindSlash>(), Projectile.damage, Projectile.knockBack / 2, Projectile.owner, 1f, 1f);
				}
			}
        }
		...
		private void OnHitEffects(bool homeIn)
		{
            if (Projectile.owner == Main.myPlayer)
            {
				for (int w = 0; w < 4; w++)
				{
					...
					Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, velocity, ModContent.ProjectileType<TurbulanceWindSlash>(), Projectile.damage / 3, Projectile.knockBack / 3, Main.myPlayer, 0f, (homeIn ? 1f : 0f));
				}
			}
		}
