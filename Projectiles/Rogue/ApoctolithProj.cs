...
namespace CalRD.Projectiles.Rogue
{
	...
	{
        public override string Texture => "CalRD/Items/Weapons/Rogue/Apoctolith";
		...
		public override void OnKill(int timeLeft)
		{
			...
			if (Projectile.Calamity().stealthStrike)
			{
				...
				while (split < 5)
				{
					...
					int shard = Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.position.X + shardspeedX, Projectile.position.Y + shardspeedY, shardspeedX, shardspeedY, ModContent.ProjectileType<ApoctolithShard>(), (int)(Projectile.damage * 0.5), Projectile.knockBack / 2f, Projectile.owner);
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			Main.spriteBatch.Draw(tex, Projectile.Center - Main.screenPosition, null, Projectile.GetAlpha(lightColor), Projectile.rotation, origin, Projectile.scale, SpriteEffects.None, 0f);
			return false;
		}

		public override void PostDraw(Color lightColor)
		{
			Texture2D tex = ModContent.Request<Texture2D>("CalRD/Items/Weapons/Rogue/ApoctolithGlow").Value;
			Vector2 origin = new Vector2(32f, 33f);
