...
namespace CalRD.Projectiles.Rogue
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/Rogue/SlickCane";
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D alternateHookTexture = Projectile.spriteDirection == -1 ? ModContent.Request<Texture2D>("CalRD/Projectiles/Rogue/SlickCaneProjectileAlt").Value : TextureAssets.Projectile[Projectile.type].Value;
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if ((target.damage > 5 || target.boss) && player.whoAmI == Main.myPlayer && !target.SpawnedFromStatue)
            {
                ...
                while (moneyValueToDrop > 10000f)
				{
					...
					DropHelper.DropItem(target.GetSource_FromThis(), target, ItemID.GoldCoin, modifiedMoneyValue);
				}
                while (moneyValueToDrop > 100f)
				{
					...
					DropHelper.DropItem(target.GetSource_FromThis(), target, ItemID.SilverCoin, modifiedMoneyValue);
				}
                while (moneyValueToDrop > 0f)
                {
					...
					DropHelper.DropItem(target.GetSource_FromThis(), target, ItemID.CopperCoin, modifiedMoneyValue);
				}
			}
        }
    }
