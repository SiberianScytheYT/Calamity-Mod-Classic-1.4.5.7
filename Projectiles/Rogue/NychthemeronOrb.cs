...
namespace CalRD.Projectiles.Rogue
{
    public class NychthemeronOrb : ModProjectile
    {
        ...
        {
            Texture2D texLight = TextureAssets.Projectile[Projectile.type].Value;
            Texture2D texDark = ModContent.Request<Texture2D>("CalRD/Projectiles/Rogue/NychthemeronOrb2").Value;
            if (Projectile.ai[0] == 0f)
            {
                Main.spriteBatch.Draw(texLight, Projectile.Center - Main.screenPosition, null, Projectile.GetAlpha(lightColor), Projectile.rotation, texLight.Size() / 2f, Projectile.scale, SpriteEffects.None, 0f);
            }
            else if (Projectile.ai[0] == 1f)
            {
                Main.spriteBatch.Draw(texDark, Projectile.Center - Main.screenPosition, null, Projectile.GetAlpha(lightColor), Projectile.rotation, texDark.Size() / 2f, Projectile.scale, SpriteEffects.None, 0f);
            }
            return false;
        }
