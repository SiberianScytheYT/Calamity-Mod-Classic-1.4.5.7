		...
		public override void AI()
		{
			...
			if (Projectile.ai[0] == 0f)
			{
				...
				if (shoot)
				{
					...
					vel.Normalize();
					vel *= 30f;
					int shard = Projectile.NewProjectile(Entity.GetSource_FromThis(), pos, vel + Projectile.velocity / 4f, ModContent.ProjectileType<FrostShardFriendly>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
			...
			else
			{
				...
				if (id >= 0 && id < Main.maxNPCs && Main.npc[id].active && !Main.npc[id].dontTakeDamage)
				{
					...
					if (shoot)
					{
						...
						vel.Normalize();
						vel *= 30f;
						int shard = Projectile.NewProjectile(Entity.GetSource_FromThis(), pos, vel + Main.npc[id].velocity, ModContent.ProjectileType<FrostShardFriendly>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
						Main.projectile[shard].alpha = Projectile.alpha;
					}
				}
				else
