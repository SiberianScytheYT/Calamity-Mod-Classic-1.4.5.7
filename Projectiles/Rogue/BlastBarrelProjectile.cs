...
namespace CalRD.Projectiles.Rogue
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/Rogue/BlastBarrel";
        ...
        public void BounceEffects()
        {
            ...
            for (int i = 0; i < projectileCount; i++)
            {
                if (Main.rand.NextBool(3))
                {
                    ...
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, Projectile.velocity + shrapnelVelocity, ModContent.ProjectileType<BarrelShrapnel>(), Projectile.damage, 3f, Projectile.owner);
                }
                else
                {
                    Vector2 fireVelocity = (Vector2.UnitY * Main.rand.NextFloat(-19f, -4f)).RotatedByRandom(MathHelper.ToRadians(40f));
                    Projectile fire = Projectile.NewProjectileDirect(Entity.GetSource_FromThis(), Projectile.Center, Projectile.velocity + fireVelocity, ModContent.ProjectileType<TotalityFire>(), (int)(Projectile.damage * 0.75f), 1f, Projectile.owner);
                    fire.timeLeft = 300;
                    fire.penetrate = 3;
                }
            }
