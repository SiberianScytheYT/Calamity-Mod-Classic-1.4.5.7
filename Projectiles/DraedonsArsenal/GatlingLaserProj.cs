...
namespace CalRD.Projectiles.DraedonsArsenal
{
	...
	{
		private SlotId gatlingLaserLoop;
			...
			if (Projectile.type == ModContent.ProjectileType<GatlingLaserProj>())
			{
				...
				if (Projectile.soundDelay <= 0)
				{
					...
					if (Projectile.ai[0] != 1f)
					{
						...
						gatlingLaserLoop = SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Item/GatlingLaserFireLoop"), Projectile.position);
					}
				}
				if (shootThisFrame && Main.myPlayer == Projectile.owner && fireLasers)
				{
					...
					if (stillInUse && hasMana && hasCharge)
					{
						...
						Projectile.NewProjectile(Entity.GetSource_FromThis(), value2.X, value2.Y, SpeedX, SpeedY, type, currentDamage, Projectile.knockBack, Projectile.owner, ai0, 0f);
					}
					else
					{
						ActiveSound result;
						if (SoundEngine.TryGetActiveSound(gatlingLaserLoop, out result))
							result.Stop();

						SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Item/GatlingLaserFireEnd"), new Vector2((int)Projectile.position.X, (int)Projectile.position.Y));
						Projectile.Kill();
					}
				}
			}
