...
namespace CalRD.Projectiles.DraedonsArsenal
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/DraedonsArsenal/PlasmaGrenade";
        ...
        {
            SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Item/PlasmaGrenadeExplosion"), new Vector2((int)Projectile.position.X, (int)Projectile.position.Y));
            if (Projectile.Calamity().stealthStrike)
            {
                if (Main.myPlayer == Projectile.owner)
                {
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<MassivePlasmaExplosion>(), Projectile.damage, Projectile.knockBack * 2f, Projectile.owner);
                }
                if (!Main.dedServ)
                {
                    for (int i = 0; i < 220; i++)
            ...
            else
            {
                ...
                if (Main.myPlayer == Projectile.owner)
                {
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<PlasmaGrenadeSmallExplosion>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                }
                if (!Main.dedServ)
                {
                    for (int i = 0; i < 120; i++)
