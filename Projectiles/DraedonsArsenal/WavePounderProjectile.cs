...
namespace CalRD.Projectiles.DraedonsArsenal
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/DraedonsArsenal/WavePounder";
        ...
        {
            SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Item/TeslaCannonFire"), Projectile.Center);
            if (Main.myPlayer == Projectile.owner)
            {
                if (!Projectile.Calamity().stealthStrike)
                {
                    for (int i = 0; i < 5; i++)
                    {
                        Projectile explosion = Projectile.NewProjectileDirect(Entity.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<WavePounderBoom>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                ...
                else
                {
                    for (int i = 0; i < 7; i++)
                    {
                        Projectile explosion = Projectile.NewProjectileDirect(Entity.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<WavePounderBoom>(), (int)(Projectile.damage * 0.3), Projectile.knockBack, Projectile.owner);
