...
namespace CalRD.Projectiles.DraedonsArsenal
{
	...
	{
        public override string Texture => "CalRD/Items/Weapons/DraedonsArsenal/Phaseslayer";
		...
		{
			SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Item/MechGaussRifle"), Projectile.Center);
			if (Main.dedServ)
				return;
		...
		private void HandleSwordBeams(Player player, CalamityGlobalItem modItem, float deltaAngle)
		{
			...
			if (Projectile.soundDelay <= 0 && deltaAngle >= 1.3f * StandardSwingSpeed)
			{
				...
				if (Main.myPlayer == player.whoAmI)
				{
					...
					Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, velocity, ModContent.ProjectileType<PhaseslayerBeam>(), (int)(Projectile.damage * SwordBeamDamageMultiplier), 0f, player.whoAmI);
				...
				Projectile.soundDelay = SwordBeamCooldown;
				SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Item/ELRFire"), Projectile.Center);
			}
		}
		...
		{
			Texture2D bladeTexture = ModContent.Request<Texture2D>("CalRD/Projectiles/DraedonsArsenal/PhaseslayerBlade").Value;
			Texture2D hiltTexture = ModContent.Request<Texture2D>(Texture).Value;
			if (IsSmall)
				bladeTexture = ModContent.Request<Texture2D>("CalRD/Projectiles/DraedonsArsenal/PhaseslayerBladeSmall").Value;
			...
			for (int i = 1; i < Projectile.oldRot.Length; i++)
			{
				...
				Main.EntitySpriteDraw(bladeTexture, Projectile.Center + bladeOffset - Main.screenPosition, afterimageFrame, color, angle, origin, Projectile.scale, SpriteEffects.None, 0f);
			}

			Main.EntitySpriteDraw(bladeTexture, Projectile.Center + bladeOffset - Main.screenPosition, frame, Color.White, Projectile.rotation + MathHelper.PiOver2, origin, Projectile.scale, SpriteEffects.None, 0f);
			Main.EntitySpriteDraw(hiltTexture, Projectile.Center - Main.screenPosition, null, lightColor, Projectile.rotation + MathHelper.PiOver2, hiltTexture.Size() * 0.5f, Projectile.scale, SpriteEffects.None, 0f);
