...
namespace CalRD.Projectiles.DraedonsArsenal
{
    ...
    {
        public override string Texture => "CalRD/Items/Weapons/DraedonsArsenal/TrackingDisk";
        ...
        public void AttemptToFireLasers(int damage)
        {
            ...
            if (Projectile.Calamity().stealthStrike)
            {
                ...
                foreach (var target in targets)
                {
                    if (targetCount >= MaxLaserCountPerShot)
                        break;
                    Projectile laser = Projectile.NewProjectileDirect(Entity.GetSource_FromThis(), Projectile.Center, 
            ...
            else
            {
                ...
                {
                    Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center, Projectile.DirectionTo(potentialTarget.Center) * 3f, ModContent.ProjectileType<TrackingDiskLaser>(), damage, Projectile.knockBack, Projectile.owner);
                }
            }
        }
