    ...
    public class CursorProj : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
			...
			for (int i = 0; i < numProj; i++)
			{
				...
				Projectile.NewProjectile(Entity.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, baseSpeed * (float)Math.Sin(offsetAngle), baseSpeed * (float)Math.Cos(offsetAngle),
