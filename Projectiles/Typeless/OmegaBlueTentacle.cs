...
namespace CalRD.Projectiles.Typeless
{
    public class OmegaBlueTentacle : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            {
                int healAmount = 10 * Projectile.damage / Projectile.damage; //should always be around max, less if enemy has defense/DR
                if (healAmount > 0)
                {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D segmentSprite = ModContent.Request<Texture2D>("CalRD/ExtraTextures/Chains/OmegaBlueTentacleSegment").Value;
            for (int i = 0; i < 6; i++)
            {
