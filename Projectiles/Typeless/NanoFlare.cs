    ...
    public class NanoFlare : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                {
                    case 1: int idx1 = Gore.NewGore(Entity.GetSource_FromThis(), goreVec, default, Main.rand.Next(61, 64), 1f);
                            Main.gore[idx1].velocity *= smokeScale;
                            Main.gore[idx1].velocity.X += 1f;
                            Main.gore[idx1].velocity.Y += 1f;
                            break;

                    case 2: int idx2 = Gore.NewGore(Entity.GetSource_FromThis(), goreVec, default, Main.rand.Next(61, 64), 1f);
                            Main.gore[idx2].velocity *= smokeScale;
                            Main.gore[idx2].velocity.X -= 1f;
                            Main.gore[idx2].velocity.Y += 1f;
                            break;

                    case 3: int idx3 = Gore.NewGore(Entity.GetSource_FromThis(), goreVec, default, Main.rand.Next(61, 64), 1f);
                            Main.gore[idx3].velocity *= smokeScale;
                            Main.gore[idx3].velocity.X += 1f;
                            Main.gore[idx3].velocity.Y -= 1f;
                            break;

                    case 4: int idx4 = Gore.NewGore(Entity.GetSource_FromThis(), goreVec, default, Main.rand.Next(61, 64), 1f);
