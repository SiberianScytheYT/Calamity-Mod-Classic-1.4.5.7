...
namespace CalRD.Tiles.Abyss
{
    public class HardenedSulphurousSandstone : ModTile
    {
        public override void SetStaticDefaults()
        {
            ...
            DustType = 32;
            // ItemDrop/* tModPorter Note: Removed. Tiles and walls will drop the item which places them automatically. Use RegisterItemDrop to alter the automatic drop if necessary. */ = ModContent.ItemType<Items.Placeables.HardenedSulphurousSandstone>();
        ...
        public override void RandomUpdate(int i, int j)
        {
            ...
            if (Main.tile[i, j + 1] != null && nearbyVineCount < 5)
            {
                if (!Main.tile[i, j + 1].HasTile && Main.tile[i, j + 1].TileType != (ushort)ModContent.TileType<SulphurousVines>())
                {
                    ...
                    Main.tile[i, j].Get<TileWallWireStateData>().Slope = 0;
