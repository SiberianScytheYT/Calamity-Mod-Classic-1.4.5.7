...
namespace CalRD.Tiles.Abyss
{
    public class PlantyMush : ModTile
    {
        public override void SetStaticDefaults()
        {
            ...
            DustType = 2;
            // ItemDrop/* tModPorter Note: Removed. Tiles and walls will drop the item which places them automatically. Use RegisterItemDrop to alter the automatic drop if necessary. */ = ModContent.ItemType<Items.Placeables.PlantyMush>();
            ...
            if (Main.tile[i, j + 1] != null)
            {
                if (!Main.tile[i, j + 1].HasTile && Main.tile[i, j + 1].TileType != (ushort)ModContent.TileType<ViperVines>())
                {
                    if (Main.tile[i, j + 1].LiquidAmount == 255 &&
                        (Main.tile[i, j + 1].WallType == (ushort)ModContent.WallType<MossyGravelWall>() ||
