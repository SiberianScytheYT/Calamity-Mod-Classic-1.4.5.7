...
namespace CalRD.Tiles.Plates
{
    public class Cinderplate : ModTile
    {
        ...
        public override void SetStaticDefaults()
        {
            if (!Main.dedServ)
            {
                PulseTexture = ModContent.Request<Texture2D>("CalRD/Tiles/Plates/CinderplatePulse").Value;
                PulseColors = new Color[PulseTexture.Width];
                Main.QueueMainThreadAction(() => PulseTexture.GetData(PulseColors));
                GlowTexture = ModContent.Request<Texture2D>("CalRD/Tiles/Plates/CinderplateGlow").Value;
            }
            ...
            MineResist = 1f;
            MinPick = 65;
            // ItemDrop/* tModPorter Note: Removed. Tiles and walls will drop the item which places them automatically. Use RegisterItemDrop to alter the automatic drop if necessary. */ = ModContent.ItemType<Items.Placeables.Plates.Cinderplate>();
