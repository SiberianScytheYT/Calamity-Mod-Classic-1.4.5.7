...
namespace CalRD.Tiles.FurnitureAstral
{
    public class MonolithChest : ModTile
    {
        public override void SetStaticDefaults()
        {
            ...
            // ItemDrop/* tModPorter Note: Removed. Tiles and walls will drop the item which places them automatically. Use RegisterItemDrop to alter the automatic drop if necessary. */ = ModContent.ItemType<Items.Placeables.FurnitureAstral.MonolithChest>();
        }
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            Texture2D glowmask = ModContent.Request<Texture2D>("CalRD/Tiles/FurnitureAstral/MonolithChestGlow").Value;
        ...
        public override bool RightClick(int i, int j)
        {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                ...
                if (chest >= 0)
                {
                    if (player.chest < 0)
                    {
                        SoundEngine.PlaySound(SoundID.NPCDeath22.WithVolumeScale(0.5f));
                    }
                }
            }
