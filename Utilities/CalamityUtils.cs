...
namespace CalRD
{
	public static class CalamityUtils
	{
		...
		public static float MeleeDamage(this Player player) => player.GetDamage(DamageClass.Generic).Additive + player.GetDamage(DamageClass.Melee).Additive - 1f;
		public static float RangedDamage(this Player player) => player.GetDamage(DamageClass.Generic).Additive + player.GetDamage(DamageClass.Ranged).Additive - 1f;
		public static float MagicDamage(this Player player) => player.GetDamage(DamageClass.Generic).Additive + player.GetDamage(DamageClass.Magic).Additive - 1f;
		public static float MinionDamage(this Player player) => player.GetDamage(DamageClass.Generic).Additive + player.GetDamage(DamageClass.Summon).Additive - 1f;
		public static float ThrownDamage(this Player player) => player.GetDamage(DamageClass.Generic).Additive + player.GetDamage(DamageClass.Throwing).Additive - 1f;
		public static float RogueDamage(this Player player) => player.GetDamage(DamageClass.Generic).Additive + player.GetDamage(DamageClass.Throwing).Additive + player.Calamity().throwingDamage - 2f;
		public static float AverageDamage(this Player player) => player.GetDamage(DamageClass.Generic).Additive + (player.GetDamage(DamageClass.Melee).Additive + player.GetDamage(DamageClass.Ranged).Additive + player.GetDamage(DamageClass.Magic).Additive + player.GetDamage(DamageClass.Summon).Additive + player.Calamity().throwingDamage - 5f) / 5f;
		...
		public static Projectile ProjectileRain(IEntitySource source, Vector2 targetPos, float xLimit, float xVariance, float yLimitLower, float yLimitUpper, float projSpeed, int projType, int damage, float knockback, int owner, int forceType = 0, int immunitySetting = 0, int cooldown = 10)
		{
			...
			Projectile proj = Projectile.NewProjectileDirect(source, spawnPosition, velocity, projType, damage, knockback, owner, 0f, 0f);
		...
		public static Projectile ProjectileBarrage(IEntitySource source, Vector2 originVec, Vector2 targetPos, bool fromRight, float xOffsetMin, float xOffsetMax, float yOffsetMin, float yOffsetMax, float projSpeed, int projType, int damage, float knockback, int owner, bool clamped = false, float inaccuracyOffset = 5f)
		{
			...
			Projectile proj = Projectile.NewProjectileDirect(source, spawnPosition, velocity, projType, damage, knockback, owner);
			return proj;
		}
		...
		{
			Texture2D slotBackgroundTex = ModContent.Request<Texture2D>("CalRD/ExtraTextures/UI/PowerCellSlot_Empty", AssetRequestMode.ImmediateLoad).Value;

			// This check is done twice because the draw order matters. We want to draw the background icon before any text.
			if (item.stack > 0)
				slotBackgroundTex = ModContent.Request<Texture2D>("CalRD/ExtraTextures/UI/PowerCellSlot_Filled", AssetRequestMode.ImmediateLoad).Value;
		...
		/// </summary>
		/// <param name="entitySource">The entity the gores belong to</param>
		...
		public static void ExplosionGores (IEntitySource entitySource, Vector2 goreSource, int goreAmt)
		{
			...
			for (int goreIndex = 0; goreIndex < goreAmt; goreIndex++)
			{
				...
				int smoke = Gore.NewGore(entitySource,source, default, Main.rand.Next(61, 64), 1f);
				Gore gore = Main.gore[smoke];
				gore.velocity *= velocityMult;
				gore.velocity.X += 1f;
				gore.velocity.Y += 1f;
				smoke = Gore.NewGore(entitySource, source, default, Main.rand.Next(61, 64), 1f);
				gore.velocity *= velocityMult;
				gore.velocity.X -= 1f;
				gore.velocity.Y += 1f;
				smoke = Gore.NewGore(entitySource, source, default, Main.rand.Next(61, 64), 1f);
		...
		public static Item LoadModItem(TagCompound tag, int itemIndex = 0)
		{
			...
			int type = ModLoader.GetMod(modName)?.Find<ModItem>(itemName).Type ?? ItemID.None;
			if (type > ItemID.None)
			{
