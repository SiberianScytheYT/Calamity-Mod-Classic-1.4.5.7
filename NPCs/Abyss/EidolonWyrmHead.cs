    ...
    public class EidolonWyrmHead : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (detectsPlayer)
            {
                ...
                {
                    NPC.soundDelay = 420;
                    SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/WyrmScream"), NPC.position);
                }
            }
            else
            {
                ...
                if (!TailSpawned && NPC.ai[0] == 0f)
                {
                    ...
                    for (int num36 = 0; num36 < maxLength; num36++)
                    {
                        ...
                        if (num36 >= 0 && num36 < minLength)
                        {
                            if (num36 % 2 == 0)
                            {
                                lol = NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), ModContent.NPCType<EidolonWyrmBody>(), NPC.whoAmI);
                            }
                            else
                            {
                                lol = NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), ModContent.NPCType<EidolonWyrmBodyAlt>(), NPC.whoAmI);
                            }
                        }
                        else
                        {
                            lol = NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), ModContent.NPCType<EidolonWyrmTail>(), NPC.whoAmI);
                        }
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            vector -= new Vector2((float)ModContent.Request<Texture2D>("CalRD/NPCs/Abyss/EidolonWyrmHeadGlow").Value.Width, (float)(ModContent.Request<Texture2D>("CalRD/NPCs/Abyss/EidolonWyrmHeadGlow").Value.Height / Main.npcFrameCount[NPC.type])) * 1f / 2f;
            ...
            Main.spriteBatch.Draw(ModContent.Request<Texture2D>("CalRD/NPCs/Abyss/EidolonWyrmHeadGlow").Value, vector,
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            ...
            if (NPC.life <= 0)
            {
                if (Main.netMode != NetmodeID.Server)
                {
                    ...
                    Gore.NewGore(NPC.GetSource_FromThis(), NPC.position, NPC.velocity, Mod.Find<ModGore>("Wyrm").Type, 1f);
                }
            }
        }
