...
namespace CalRD.NPCs.Abyss
{
    public class ColossalSquid : ModNPC
    {
            ...
            else
            {
                ...
                if (hasBeenHit)
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient && NPC.localAI[0] >= 150f)
                    {
                        ...
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center.X, NPC.Center.Y + 60, 0f, 2f, ModContent.ProjectileType<InkBombHostile>(), damage, 0f, Main.myPlayer, 0f, 0f);
                    }
                    NPC.rotation = NPC.velocity.X * 0.05f;
                    NPC.velocity *= 0.975f;
                    float num263 = 2.5f;
        ...
        {
            target.AddBuff(ModContent.BuffType<CrushDepth>(), 600, true);
        ...
        public override void OnKill()
        {
            ...
            DropHelper.DropItem(NPC.GetSource_FromThis(), NPC, ItemID.BlackInk, 3, 5);
            DropHelper.DropItemCondition(NPC.GetSource_FromThis(), NPC, ModContent.ItemType<DepthCells>(), CalamityWorld.downedCalamitas, 2, 26, 38);
            ...
            DropHelper.DropItemCondition(NPC.GetSource_FromThis(), NPC, ModContent.ItemType<CalamarisLament>(), CalamityWorld.downedPolterghast, 3, 1, 1);
            DropHelper.DropItemChance(NPC.GetSource_FromThis(), NPC, ModContent.ItemType<InkBomb>(), 10, 1, 1);
        }
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            ...
            if (NPC.life <= 0)
            {
                if (Main.netMode != NetmodeID.Server)
                {
                    ...
                    Gore.NewGore(NPC.GetSource_FromThis(), NPC.position, NPC.velocity, Mod.Find<ModGore>("ColossalSquid").Type, 1f);
                    Gore.NewGore(NPC.GetSource_FromThis(), NPC.position, NPC.velocity, Mod.Find<ModGore>("ColossalSquid2").Type, 1f);
                    Gore.NewGore(NPC.GetSource_FromThis(), NPC.position, NPC.velocity, Mod.Find<ModGore>("ColossalSquid3").Type, 1f);
                    Gore.NewGore(NPC.GetSource_FromThis(), NPC.position, NPC.velocity, Mod.Find<ModGore>("ColossalSquid4").Type, 1f);
                }
            }
        }
    }
