    ...
    public class SoulSeeker : ModNPC
    {
            ...
            if (timer > 60)
            {
                if (Main.netMode != NetmodeID.MultiplayerClient && Main.rand.NextBool(10) && parent.ai[1] < 2f)
                {
                    ...
                    if (NPC.CountNPCS(npcType) < 3)
                        NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.Center.X, (int)NPC.Center.Y, npcType);
					...
					Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, velocity, type, damage, 1f, NPC.target, 1f, 0f);
                }
                timer = 0;
            }
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            ...
            if (NPC.life <= 0)
            {
	            ...
	            {
		            Gore.NewGore(NPC.GetSource_FromThis(), NPC.position, NPC.velocity, Mod.Find<ModGore>("SoulSeeker").Type, 1f);
		            Gore.NewGore(NPC.GetSource_FromThis(), NPC.position, NPC.velocity, Mod.Find<ModGore>("SoulSeeker2").Type, 1f);
		            Gore.NewGore(NPC.GetSource_FromThis(), NPC.position, NPC.velocity, Mod.Find<ModGore>("SoulSeeker3").Type, 1f);
                }
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
		{
			...
			texture = ModContent.Request<Texture2D>("CalRD/NPCs/Calamitas/SoulSeekerGlow").Value;
