...
namespace CalRD.NPCs.Astral
{
    public class Hadarian : ModNPC
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            if (!Main.dedServ)
                glowmask = ModContent.Request<Texture2D>("CalRD/NPCs/Astral/HadarianGlow").Value;
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            ...
            if (NPC.life <= 0)
            {
                if (Main.netMode != NetmodeID.Server)
                {
                    for (int i = 0; i < 5; i++)
                    {
                        Gore.NewGore(NPC.GetSource_FromThis(), NPC.Center, NPC.velocity * 0.3f, Mod.Find<ModGore>("HadarianGore" + i).Type);
                    }
                }
            }
        }
