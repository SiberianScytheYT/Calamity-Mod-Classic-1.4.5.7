        ...
        public override void AI()
        {
            ...
            if (Collision.CanHit(target.position, target.width, target.height, NPC.position, NPC.width, NPC.height))
            {
                ...
                if (NPC.ai[1] >= (CalamityWorld.death ? 120f : 160f))
                {
                    ...
                    int n = NPC.NewNPC(NPC.GetSource_FromThis(), (int)spawnPoint.X, (int)spawnPoint.Y, ModContent.NPCType<AstralSeekerSpit>());
            ...
            if (NPC.life <= 0)
            {
                if (Main.netMode != NetmodeID.Server)
                {
                    for (int i = 0; i < 5; i++)
                    {
                        ...
                        Gore.NewGore(NPC.GetSource_FromThis(), NPC.position + new Vector2(Main.rand.NextFloat(0f, NPC.width), Main.rand.NextFloat(0f, NPC.height)), NPC.velocity * rand, Mod.Find<ModGore>("BigSightseerGore" + i).Type);
                    }
                }
            }
        }
