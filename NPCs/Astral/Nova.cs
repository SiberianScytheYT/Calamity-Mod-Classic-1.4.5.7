...
namespace CalRD.NPCs.Astral
{
    public class Nova : ModNPC
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            if (!Main.dedServ)
                glowmask = ModContent.Request<Texture2D>("CalRD/NPCs/Astral/NovaGlow").Value;
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            ...
            if (NPC.life <= 0)
            {
                if (Main.netMode != NetmodeID.Server)
                {
                    for (int i = 0; i < 7; i++)
                    {
                        Gore.NewGore(NPC.GetSource_FromThis(), NPC.Center, NPC.velocity * 0.3f, Mod.Find<ModGore>("NovaGore" + i).Type);
                    }
                }
            }
        }
        ...
        public override void OnKill()
        {
            ...
            DropHelper.DropItemCondition(NPC.GetSource_FromThis(), NPC, ModContent.ItemType<StellarCannon>(), CalamityWorld.downedAstrageldon, 7, 1, 1);
        }
    }
}
