...
namespace CalRD.NPCs
{
	public class CalamityGlobalNPC : GlobalNPC
    {
        ...
        public override void SetDefaults(NPC npc)
        {
			...
			if (CalRD.DRValues.ContainsKey(npc.type))
			{
				CalRD.DRValues.TryGetValue(npc.type, out float newDR);
				DR = newDR;
			}

			if (CalRD.bossKillTimes.ContainsKey(npc.type))
			{
				CalRD.bossKillTimes.TryGetValue(npc.type, out int revKillTime);
        ...
        public override void ApplyDifficultyAndPlayerScaling(NPC npc, int numPlayers, float balance, float bossAdjustment)/* tModPorter Note: balance -> balance (bossAdjustment is different, see the docs for details) */
        {
            ...
            bool isCalamityNPC = npc.ModNPC != null && npc.ModNPC.Mod == CalRD.Instance;
        ...
        public override void ModifyIncomingHit(NPC npc, ref NPC.HitModifiers modifiers)
        {
            ...
            if (CalamityWorld.revenge || BossRushEvent.BossRushActive)
            {
                ...
                {
                    if (npc.life - modifiers.SourceDamage.Base <= 0)
                    {
                        if (newAI[0] != 1f)
                        {
                            newAI[0] = 1f;
            ...
            double yellowCandleDamage = 0.05 * modifiers.SourceDamage.Base;
            ...
            if (yellowCandle > 0 && DR < 0.99f && npc.takenDamageMultiplier > 0.05f)
	            modifiers.SourceDamage.Base += (float)yellowCandleDamage;
            ...
            //modifiers.SourceDamage = Main.CalculateDamage((int)modifiers.SourceDamage, -npc.defense);
        }
        ...
        public override void ModifyHitByProjectile(NPC npc, Projectile projectile, ref NPC.HitModifiers modifiers)
        {
			...
			CalamityGlobalTownNPC.MakeTownNPCsTakeMoreDamage(npc, projectile, Mod, ref projectile.damage);

			if (!projectile.npcProj && !projectile.trap)
			{
				if (projectile.CountsAsClass(DamageClass.Ranged) && modPlayer.plagueReaper && pFlames > 0)
				{
			...
			if (Main.expertMode)
			{
				if (AstrumDeusIDs.Contains(npc.type))
				{
					GrenadeResist(projectile, ref projectile.damage);
					PierceResistGlobal(projectile, ref projectile.damage);
		...
		public override void OnHitByItem(NPC npc, Player player, Item item, NPC.HitInfo hit, int damageDone)
        {
            if (player.Calamity().bloodflareSet)
            {
                    ...
                    player.Calamity().bloodflareHeartTimer <= 0)
                {
                    ...
                    DropHelper.DropItem(npc.GetSource_FromThis(), npc, ItemID.Heart);
                }
                    ...
                    player.Calamity().bloodflareManaTimer <= 0)
                {
                    ...
                    DropHelper.DropItem(npc.GetSource_FromThis(), npc, ItemID.Star);
                }
            }
        }
        #endregion
        ...
        public override bool CheckDead(NPC npc)
        {
                ...
                Main.player[npc.target].ZoneDungeon)
            {
                ...
                if (Main.rand.NextBool(maxValue) && Main.wallDungeon[Main.tile[(int)npc.Center.X / 16, (int)npc.Center.Y / 16].WallType])
                {
                    ...
                    NPC.NewNPC(npc.GetSource_FromThis(), (int)npc.Center.X, (int)npc.Center.Y, randomType, 0, 0f, 0f, 0f, 0f, 255);
                }
            }
                    ...
                    case NPCID.MotherSlime:
                        if (npc.life <= 0)
                        {
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                ...
                                {
                                    int slime = NPC.NewNPC(npc.GetSource_FromThis(), (int)npc.Center.X, (int)(npc.position.Y + npc.height), NPCID.BlueSlime, 0, 0f, 0f, 0f, 0f, 255);
        ...
        public override bool PreDraw(NPC npc, SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
			if (npc.type != NPCID.BrainofCthulhu && (npc.type != NPCID.DukeFishron || npc.ai[0] <= 9f))
			{
				if (CalamityConfig.Instance.DebuffDisplay && (npc.boss || BossHealthBarManager.MinibossHPBarList.Contains(npc.type) || BossHealthBarManager.OneToMany.ContainsKey(npc.type) || CalamityLists.needsDebuffIconDisplayList.Contains(npc.type)))
				{
					...
					if (aFlames > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/DamageOverTime/AbyssalFlames").Value);
					if (astralInfection > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/DamageOverTime/AstralInfectionDebuff").Value);
					if (bFlames > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/DamageOverTime/BrimstoneFlames").Value);
					if (bBlood > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/DamageOverTime/BurningBlood").Value);
					if (cDepth > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/DamageOverTime/CrushDepth").Value);
					if (dFlames > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/DamageOverTime/DemonFlames").Value);
					if (gsInferno > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/DamageOverTime/GodSlayerInferno").Value);
					if (hFlames > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/DamageOverTime/HolyFlames").Value);
					if (nightwither > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/DamageOverTime/Nightwither").Value);
					if (pFlames > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/DamageOverTime/Plague").Value);
					if (shellfishVore > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/DamageOverTime/ShellfishEating").Value);
					if (pShred > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/DamageOverTime/Shred").Value);
					if (clamDebuff > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/DamageOverTime/SnapClamDebuff").Value);
					if (sulphurPoison > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/DamageOverTime/SulphuricPoisoning").Value);
					if (vaporfied > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/DamageOverTime/Vaporfied").Value);

					// Stat debuffs
					if (aCrunch > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/StatDebuffs/ArmorCrunch").Value);
					if (enraged > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/StatDebuffs/Enraged").Value);
					if (eutrophication > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/StatDebuffs/Eutrophication").Value);
					if (eFreeze > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/StatDebuffs/ExoFreeze").Value);
					if (gState > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/StatDebuffs/GlacialState").Value);
					if (irradiated > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/StatDebuffs/Irradiated").Value);
					if (marked > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/StatDebuffs/MarkedforDeath").Value);
					if (pearlAura > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/StatDebuffs/PearlAura").Value);
                    if (relicOfResilienceWeakness > 0)
                        buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/StatDebuffs/ProfanedWeakness").Value);
                    if (silvaStun > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/StatDebuffs/SilvaStun").Value);
					if (tSad > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/StatDebuffs/TemporalSadness").Value);
					if (tesla > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/StatDebuffs/TeslaFreeze").Value);
					if (timeSlow > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/StatDebuffs/TimeSlow").Value);
					if (wCleave > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/StatDebuffs/WarCleave").Value);
					if (wDeath > 0)
						buffTextureList.Add(Request<Texture2D>("CalRD/Buffs/StatDebuffs/WhisperingDeath").Value);
        ...
        public override void PostDraw(NPC npc, SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            if (CalamityWorld.revenge || BossRushEvent.BossRushActive)
            {
                ...
                if (npc.type == NPCID.SkeletronPrime)
                {
                    ...
                    spriteBatch.Draw(TextureAssets.BoneEyes.Value, npc.Center - Main.screenPosition + new Vector2(0, npc.gfxOffY),
				...
				else if (DestroyerIDs.Contains(npc.type))
				{
					if (drawColor != Color.Black && npc.ai[2] == 0f && ((newAI[3] >= 900f && npc.life / (float)npc.lifeMax < 0.5f) || (newAI[1] < 600f && newAI[1] > 60f)))
					{
						...
						for (int i = 0; i < 3; i++)
						{
							spriteBatch.Draw(TextureAssets.Dest[npc.type - NPCID.TheDestroyer].Value, npc.Center - Main.screenPosition + new Vector2(0, npc.gfxOffY), npc.frame,
					...
					if (plr == Main.myPlayer && projectile.ai[0] == 0f)
					{
                        ...
                        if (Main.myPlayer == projectile.owner)
                        {
                            if (Main.netMode == NetmodeID.SinglePlayer)
                            {
                                ...
                                int oldDuke = NPC.NewNPC(new EntitySource_BossSpawn(proj), (int)proj.Center.X, (int)proj.Center.Y + 100, NPCType<OldDuke.OldDuke>());
                                CalamityUtils.BossAwakenMessage(oldDuke);
                            }
							else
                            {
                                var netMessage = CalRD.Instance.GetPacket();
                                netMessage.Write((byte)CalRDMessageType.ServersideSpawnOldDuke);
