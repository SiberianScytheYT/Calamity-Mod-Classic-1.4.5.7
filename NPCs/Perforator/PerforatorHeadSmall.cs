...
namespace CalRD.NPCs.Perforator
{
    ...
    public class PerforatorHeadSmall : ModNPC
    {
			...
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				if (!TailSpawned)
				{
					...
					for (int num36 = 0; num36 < maxLength; num36++)
					{
						...
						{
							lol = NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), ModContent.NPCType<PerforatorBodySmall>(), NPC.whoAmI);
						}
						else
						{
							lol = NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), ModContent.NPCType<PerforatorTailSmall>(), NPC.whoAmI);
						}
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
		{
			...
			texture2D15 = ModContent.Request<Texture2D>("CalRD/NPCs/Perforator/PerforatorHeadSmallGlow").Value;
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            ...
            if (NPC.life <= 0)
            {
                if (Main.netMode != NetmodeID.Server)
                {
                    ...
                    Gore.NewGore(NPC.GetSource_FromThis(), NPC.position, NPC.velocity, Mod.Find<ModGore>("SmallPerf").Type, 1f);
                }
            }
        }
        ...
        {
			DropHelper.DropItem(NPC.GetSource_FromThis(), NPC, ModContent.ItemType<BloodSample>(), 2, 6);
			DropHelper.DropItem(NPC.GetSource_FromThis(), NPC, ItemID.CrimtaneBar, 1, 3);
			DropHelper.DropItem(NPC.GetSource_FromThis(), NPC, ItemID.Vertebrae, 1, 2);
        }
