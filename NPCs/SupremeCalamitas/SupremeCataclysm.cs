    ...
    public class SupremeCataclysm : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (NPC.localAI[0] >= 120f)
            {
                ...
                {
                    NPC.ai[1] = 0f;
					SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/SCalSounds/BrimstoneSkullSound"), NPC.Center);
					int type = ModContent.ProjectileType<BrimstoneWave>();
					int damage = NPC.GetProjectileDamage(type);
					if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, new Vector2(-8f, 0f), type, damage, 0f, Main.myPlayer);
                    }
                }
                NPC.ai[2] += 1f;
                if (deadBrother)
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
		{
			...
			texture2D15 = ModContent.Request<Texture2D>("CalRD/NPCs/SupremeCalamitas/SupremeCataclysmGlow").Value;
