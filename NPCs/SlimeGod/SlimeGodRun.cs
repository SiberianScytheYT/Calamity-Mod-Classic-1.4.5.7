...
namespace CalRD.NPCs.SlimeGod
{
    ...
    public class SlimeGodRun : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            NPC.scale = 1.1f;
            NPC.defense = 12;
            NPC.LifeMaxNERB(4150, 5700, 1600000);
		...
		public override void AI()
        {
			...
			if (lifeRatio <= 0.5f && Main.netMode != NetmodeID.MultiplayerClient && expertMode)
            {
                ...
                NPC.NewNPC(NPC.GetSource_FromThis(), (int)spawnAt.X - 30, (int)spawnAt.Y, ModContent.NPCType<SlimeGodRunSplit>());
                NPC.NewNPC(NPC.GetSource_FromThis(), (int)spawnAt.X + 30, (int)spawnAt.Y, ModContent.NPCType<SlimeGodRunSplit>());
                NPC.active = false;
                NPC.netUpdate = true;
                return;
            ...
            else if (NPC.ai[0] == 1f)
            {
                ...
                if (NPC.velocity.Y == 0f)
                {
                    ...
                    NPC.ai[1] += 1f;
                    float num1879 = 50f;
                    float num1880 = BossRushEvent.BossRushActive ? 16f : death ? 4.5f : revenge ? 4f : expertMode ? 3.5f : 3f;
					if (revenge)
					{
        ...
        {
            target.AddBuff(BuffID.Cursed, 90, true);
        }
    }
}
