    ...
    public class GiantClam : ModNPC
    {
            ...
            if (hitAmount == 5)
            {
                ...
                else
                {
                    ...
                    else if (attack == 0)
                    {
                        ...
                        if (NPC.ai[1] >= 90f)
                        {
                            ...
                            NPC.NewNPC(NPC.GetSource_FromThis(), (int)(NPC.Center.X + 5), (int)NPC.Center.Y, ModContent.NPCType<Clam>(), 0, 0f, 0f, 0f, 0f, 255);
                            NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<Clam>(), 0, 0f, 0f, 0f, 0f, 255);
                            NPC.NewNPC(NPC.GetSource_FromThis(), (int)(NPC.Center.X - 5), (int)NPC.Center.Y, ModContent.NPCType<Clam>(), 0, 0f, 0f, 0f, 0f, 255);
                        }
                    }
                    else if (attack == 1)
                    {
                        ...
                        else if (NPC.ai[2] == 4f)
                        {
                            if (NPC.velocity.Y == 0f)
                            {
                                ...
                                NPC.noGravity = false;
                                attack = -1;
                                SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Item/ClamImpact"), NPC.position);
                                for (int stompDustArea = (int)NPC.position.X - 30; stompDustArea < (int)NPC.position.X + NPC.width + 60; stompDustArea += 30)
                                {
                                    ...
                                    int stompGore = Gore.NewGore(Entity.GetSource_FromThis(), new Vector2((float)(stompDustArea - 30), NPC.position.Y + (float)NPC.height - 12f), default, Main.rand.Next(61, 64), 1f);
                ...
                else if (Main.hardMode)
                {
                    ...
                    else if (attack == 2)
                    {
                        ...
                        num6 *= num8;
                        num7 *= num8;
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center.X, NPC.Center.Y, num6, num7, projectileShot, damage, 0f, Main.myPlayer, 0f, 0f);
                        for (int i = 0; i < 4; i++)
                        {
                            offsetAngle = startAngle + deltaAngle * (i + i * i) / 2f + 32f * i;
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), value9.X, value9.Y, (float)(Math.Sin(offsetAngle) * 3f), (float)(Math.Cos(offsetAngle) * 3f), projectileShot, damage, 0f, Main.myPlayer, 0f, 0f);
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), value9.X, value9.Y, (float)(-Math.Sin(offsetAngle) * 3f), (float)(-Math.Cos(offsetAngle) * 3f), projectileShot, damage, 0f, Main.myPlayer, 0f, 0f);
                        }
                        attack = -1;
                        NPC.ai[3] = 0f;
                    ...
                    else if (attack == 3)
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            for (int i = 0; i < 11; i++)
                            {
                                Projectile.NewProjectile(Entity.GetSource_FromThis(), player.Center.X + shotSpacing, player.Center.Y - 750f, 0f, 8f, ModContent.ProjectileType<PearlRain>(), damage, 0f, Main.myPlayer, 0f, 0f);
                                shotSpacing -= 150f;
                            }
                        }
                        attack = -1;
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            ...
            if (NPC.life <= 0)
            {
                ...
                if (Main.netMode != NetmodeID.Server)
                {
                    Gore.NewGore(NPC.GetSource_FromThis(), NPC.position, NPC.velocity, Mod.Find<ModGore>("GiantClam1").Type, 1f);
                    Gore.NewGore(NPC.GetSource_FromThis(), NPC.position, NPC.velocity, Mod.Find<ModGore>("GiantClam2").Type, 1f);
                    Gore.NewGore(NPC.GetSource_FromThis(), NPC.position, NPC.velocity, Mod.Find<ModGore>("GiantClam3").Type, 1f);
                    Gore.NewGore(NPC.GetSource_FromThis(), NPC.position, NPC.velocity, Mod.Find<ModGore>("GiantClam4").Type, 1f);
                    Gore.NewGore(NPC.GetSource_FromThis(), NPC.position, NPC.velocity, Mod.Find<ModGore>("GiantClam5").Type, 1f);
                }
            }
        }
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            vector -= new Vector2((float)ModContent.Request<Texture2D>("CalRD/NPCs/SunkenSea/GiantClamGlow").Value.Width, (float)(ModContent.Request<Texture2D>("CalRD/NPCs/SunkenSea/GiantClamGlow").Value.Height / Main.npcFrameCount[NPC.type])) * 1f / 2f;
            ...
            Main.spriteBatch.Draw(ModContent.Request<Texture2D>("CalRD/NPCs/SunkenSea/GiantClamGlow").Value, vector,
