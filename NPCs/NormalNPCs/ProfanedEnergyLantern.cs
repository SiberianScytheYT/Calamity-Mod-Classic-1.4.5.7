                ...
                if (!Main.player[NPC.target].dead)
                {
                    ...
                    if (NPC.localAI[1] >= 600f)
                    {
                        if (!Collision.SolidCollision(NPC.position, NPC.width, NPC.height))
                        {
                            ...
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center.X, NPC.Center.Y, num189, num190, num9, 40, 0f, Main.myPlayer, 0f, 0f);
                            NPC.localAI[1] = 0f;
                            return;
                        }
                        NPC.localAI[1] = 250f;
            ...
            if (CalamityGlobalNPC.energyFlame != -1)
            {
                ...
                while (draw)
                {
                    ...
                    else
                    {
						...
						Texture2D chain = ModContent.Request<Texture2D>("CalRD/NPCs/NormalNPCs/ProfanedEnergySegment").Value;
