			...
			if (phase2)
            {
                ...
                else
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient && NPC.ai[2] == 20f)
                    {
                        ...
                        num147 *= num149;
                        num148 *= num149;
                        int proj = Projectile.NewProjectile(NPC.GetSource_FromThis(), vector17.X, vector17.Y, num147, num148, type, damage, 0f, Main.myPlayer, 0f, 0f);
                    }
                }
                return;
            }
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
		{
			...
			if (Main.npc[CalamityGlobalNPC.ghostBoss].active && !phase2)
			{
				...
				while (draw)
				{
					...
					Main.spriteBatch.Draw(ModContent.Request<Texture2D>("CalRD/NPCs/Polterghast/PolterghastChain").Value, new Vector2(center.X - Main.screenPosition.X, center.Y - Main.screenPosition.Y),
						new Microsoft.Xna.Framework.Rectangle?(new Rectangle(0, 0, ModContent.Request<Texture2D>("CalRD/NPCs/Polterghast/PolterghastChain").Value.Width, chainWidth)), color2, rotation2,
						new Vector2(ModContent.Request<Texture2D>("CalRD/NPCs/Polterghast/PolterghastChain").Value.Width * 0.5f, ModContent.Request<Texture2D>("CalRD/NPCs/Polterghast/PolterghastChain").Value.Height * 0.5f), 1f, SpriteEffects.None, 0f);
				}
			}
			...
			texture2D15 = ModContent.Request<Texture2D>("CalRD/NPCs/Polterghast/PolterghastHookGlow").Value;
