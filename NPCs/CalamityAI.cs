    ...
    public class CalamityAI
    {
						...
						if (calamityGlobalNPC.newAI[3] % 40f == 0f)
						{
							...
							if (Main.netMode != NetmodeID.MultiplayerClient)
							{
								...
								for (int i = 0; i < numProj; i++)
								{
									...
									int proj = Projectile.NewProjectile(npc.GetSource_FromThis(), vectorCenter.X, vectorCenter.Y, baseSpeed * (float)Math.Sin(offsetAngle), baseSpeed * (float)Math.Cos(offsetAngle), type, damage, 0f, Main.myPlayer, 0f, 0f);
				...
				if (head)
				{
					...
					if (calamityGlobalNPC.newAI[2] == 0f && npc.ai[0] == 0f)
					{
						...
						for (int num36 = 0; num36 < maxLength; num36++)
						{
							...
							{
								if (num36 % 2 == 0)
									lol = NPC.NewNPC(npc.GetSource_FromThis(), (int)npc.position.X + (npc.width / 2), (int)npc.position.Y + (npc.height / 2), ModContent.NPCType<AquaticScourgeBodyAlt>(), npc.whoAmI);
								else
									lol = NPC.NewNPC(npc.GetSource_FromThis(), (int)npc.position.X + (npc.width / 2), (int)npc.position.Y + (npc.height / 2), ModContent.NPCType<AquaticScourgeBody>(), npc.whoAmI);
							}
							else
								lol = NPC.NewNPC(npc.GetSource_FromThis(), (int)npc.position.X + (npc.width / 2), (int)npc.position.Y + (npc.height / 2), ModContent.NPCType<AquaticScourgeTail>(), npc.whoAmI);
							...
							if (Vector2.Distance(player.Center, npc.Center) > 320f)
							{
								...
								if (Main.netMode != NetmodeID.MultiplayerClient)
								{
									...
									for (int num321 = 0; num321 < num320; num321++)
									{
										...
										Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, vector15, type, damage, 0f, Main.myPlayer, 0f, Main.rand.Next(-45, 1));
									}
								}
							}
						}
						...
						if (npc.type == ModContent.NPCType<AquaticScourgeBody>())
						{
							...
							if (npc.localAI[0] >= Main.rand.Next(700, 10001))
							{
								...
								if (Collision.CanHit(npc.position, npc.width, npc.height, player.position, player.width, player.height))
								{
									...
									Projectile.NewProjectile(npc.GetSource_FromThis(), vector104.X, vector104.Y, num942, num943, type, damage, 0f, Main.myPlayer, 0f, 0f);
									npc.netUpdate = true;
								}
							}
						}
		...
		public static void BrimstoneElementalAI(NPC npc, Mod mod)
		{
			...
			else if (npc.ai[0] == 1f)
			{
				...
				if (npc.alpha >= 255)
				{
					...
					{
						NPC.NewNPC(npc.GetSource_FromThis(), (int)vectorCenter.X, (int)vectorCenter.Y, ModContent.NPCType<Brimling>());
					}
				...
				if (npc.ai[1] % divisor == divisor - 1f)
				{
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						...
						if (npc.ai[1] % divisor2 == divisor2 - 1f)
						{
							...
							{
								offsetAngle = startAngle + deltaAngle * i;
								Projectile.NewProjectile(npc.GetSource_FromThis(), vectorCenter.X, vectorCenter.Y, baseSpeed * (float)Math.Sin(offsetAngle), baseSpeed * (float)Math.Cos(offsetAngle), type, damage + (provy ? 30 : 0), 0f, Main.myPlayer, 1f, 0f);
							}
						}
						...
						damage = npc.GetProjectileDamage(type);
						int projectileShot = Projectile.NewProjectile(npc.GetSource_FromThis(), vectorCenter.X, vectorCenter.Y, relativeSpeedX, relativeSpeedY, type, damage + (provy ? 30 : 0), 0f, Main.myPlayer, 0f, 0f);
				...
				if (Main.netMode != NetmodeID.MultiplayerClient)
				{
					...
					if (npc.localAI[0] >= 120f)
					{
						...
						for (int j = 0; j < totalProjectiles; j++)
						{
							...
							int proj = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center + offset, vectorCenter, type, damage + (provy ? 30 : 0), 0f, Main.myPlayer, 1f, 0f);
						...
						for (int k = 0; k < totalProjectiles; k++)
						{
							...
							Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, vector255, type, damage + (provy ? 30 : 0), 0f, Main.myPlayer, 1f, 0f);
						}
					}
				}
			...
			else if (npc.ai[0] == 5f)
			{
				...
				else if (npc.ai[1] >= 180f)
				{
					...
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						if (npc.ai[1] == 180f)
						{
							...
							int damage = npc.GetProjectileDamage(type);
							Projectile.NewProjectile(npc.GetSource_FromThis(), source, laserVelocity2, type, damage + (provy ? 30 : 0), 0f, Main.myPlayer, 0f, npc.whoAmI);
						}
					}
				}
				else
				{
					...
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						...
						{
							Projectile.NewProjectile(npc.GetSource_FromThis(), source, laserVelocity, ModContent.ProjectileType<BrimstoneTargetRay>(), 0, 0f, Main.myPlayer, 0f, npc.whoAmI);
							calamityGlobalNPC.newAI[0] = 1f;
						}
						else
						{
							if (npc.ai[1] == 150f)
							{
								...
								Projectile.NewProjectile(npc.GetSource_FromThis(), source.X, source.Y, npc.localAI[0], npc.localAI[1], ModContent.ProjectileType<BrimstoneTargetRay>(), 0, 0f, Main.myPlayer, 1f, npc.whoAmI);
							}
						}
					}
				}
			...
			if (phase2)
			{
				...
				if (calamityGlobalNPC.newAI[1] == 0f && lifeRatio <= 0.35f && expertMode)
				{
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						...
						{
							int spawn = NPC.NewNPC(npc.GetSource_FromThis(), (int)(npc.Center.X + (Math.Sin(i * seekerSpread) * seekerDistance)), (int)(npc.Center.Y + (Math.Cos(i * seekerSpread) * seekerDistance)), ModContent.NPCType<SoulSeeker>(), npc.whoAmI, 0, 0, 0, -1);
				...
				if (npc.life > 0)
				{
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						...
						if ((npc.life + num660) < calamityGlobalNPC.newAI[0])
						{
							...
							{
								NPC.NewNPC(npc.GetSource_FromThis(), (int)npc.Center.X, (int)npc.position.Y + npc.height, ModContent.NPCType<CalamitasRun>(), npc.whoAmI);
								NPC.NewNPC(npc.GetSource_FromThis(), (int)npc.Center.X, (int)npc.position.Y + npc.height, ModContent.NPCType<CalamitasRun2>(), npc.whoAmI);
				...
				if (Main.netMode != NetmodeID.MultiplayerClient)
				{
					...
					if (phase2)
					{
						...
						if (npc.localAI[1] >= 180f)
						{
							...
							if (!Collision.CanHit(npc.position, npc.width, npc.height, player.position, player.width, player.height))
							{
								int proj = Projectile.NewProjectile(npc.GetSource_FromThis(), vector82.X, vector82.Y, num825, num826, type, damage + (provy ? 30 : 0), 0f, Main.myPlayer, player.Center.X, player.Center.Y);
								Main.projectile[proj].tileCollide = false;
							}
							else
								Projectile.NewProjectile(npc.GetSource_FromThis(), vector82.X, vector82.Y, num825, num826, type, damage + (provy ? 30 : 0), 0f, Main.myPlayer, 0f, 0f);
						}
					}
					else
					{
					...
					if (phase2)
					{
						...
						if (npc.localAI[1] >= 60f && Collision.CanHit(npc.position, npc.width, npc.height, player.position, player.width, player.height))
						{
							...
							if (!Collision.CanHit(npc.position, npc.width, npc.height, player.position, player.width, player.height))
							{
								type = ModContent.ProjectileType<BrimstoneHellfireball>();
								damage = npc.GetProjectileDamage(type);
								int proj = Projectile.NewProjectile(npc.GetSource_FromThis(), vector83.X, vector83.Y, num834, num835, type, damage + (provy ? 30 : 0), 0f, Main.myPlayer, player.Center.X, player.Center.Y);
								Main.projectile[proj].tileCollide = false;
							}
							else
								Projectile.NewProjectile(npc.GetSource_FromThis(), vector83.X, vector83.Y, num834, num835, type, damage + (provy ? 30 : 0), 0f, Main.myPlayer, 0f, 0f);
						}
					}
					else
					{
				...
				if (Collision.CanHit(npc.position, npc.width, npc.height, player.position, player.width, player.height) && fireDelay)
				{
					...
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						...
						if (npc.localAI[1] > 12f)
						{
							...
							Projectile.NewProjectile(npc.GetSource_FromThis(), vector86.X, vector86.Y, num864, num865, type, damage + (provy ? 30 : 0), 0f, Main.myPlayer, 0f, 0f);
						}
					}
				}
			}
		...
		public static void AstrumAureusAI(NPC npc, Mod mod)
        {
            ...
            else if (npc.ai[0] == 4f)
            {
                ...
                {
                    // Play stomp sound
                    SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/LegStomp"), npc.position);
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    if (npc.localAI[1] >= (240f - (death ? 60f * (1f - lifeRatio) : 0f)))
                    {
                        ...
                        if (spawnFlag && Main.netMode != NetmodeID.MultiplayerClient)
                            NPC.NewNPC(npc.GetSource_FromThis(), (int)npc.Center.X, (int)npc.Center.Y - 25, ModContent.NPCType<AureusSpawn>());
				...
				if ((halfHealth && calamityGlobalNPC.newAI[0] == 0f) || despawnRemainingWorm)
				{
					...
					if (despawning)
					{
						...
						if (Main.netMode != NetmodeID.MultiplayerClient)
						{
							...
							int headOneID = NPC.NewNPC(npc.GetSource_FromThis(), (int)npc.Center.X, (int)npc.Center.Y, npc.type, startIndexHeadOne);
							...
							if (Main.netMode == NetmodeID.Server)
							{
								...
								netMessage.Write((byte)CalRDMessageType.SyncCalamityNPCAIArray);
							...
							int headTwoID = NPC.NewNPC(npc.GetSource_FromThis(), (int)npc.Center.X, (int)npc.Center.Y, npc.type, startIndexHeadTwo);
							...
							SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/AstrumDeusSplit"), player.Center);
						}
						return;
					}
				}
				...
				if (Main.netMode != NetmodeID.MultiplayerClient)
				{
					if (npc.ai[0] == 0f)
					{
						...
						for (int num36 = 0; num36 < maxLength; num36++)
						{
							int lol;
							if (num36 >= 0 && num36 < maxLength - 1)
								lol = NPC.NewNPC(npc.GetSource_FromThis(), (int)npc.position.X + (npc.width / 2), (int)npc.position.Y + (npc.height / 2), bodyType, npc.whoAmI);
							else
								lol = NPC.NewNPC(npc.GetSource_FromThis(), (int)npc.position.X + (npc.width / 2), (int)npc.position.Y + (npc.height / 2), tailType, npc.whoAmI);
				...
				if (npc.type == ModContent.NPCType<AstrumDeusBodySpectral>() && Main.netMode != NetmodeID.MultiplayerClient)
				{
					...
					if (!flyAtTarget)
					{
						if (npc.localAI[0] % divisor == 0f && npc.ai[0] % 2f == 0f)
						{
							...
							if (Vector2.Distance(player.Center, npc.Center) > 80f)
							{
								...
								vector104.X += num942 * 5f;
								vector104.Y += num943 * 5f;
								Projectile.NewProjectile(npc.GetSource_FromThis(), vector104.X, vector104.Y, num942, num943, type, damage, 0f, Main.myPlayer, player.Center.X, player.Center.Y);
								npc.netUpdate = true;
							}
						}
					}
					else
					{
						if (npc.localAI[0] % divisor == 0f && npc.ai[0] % 2f == 0f)
						{
							...
							int damage = npc.GetProjectileDamage(type);
							int proj = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, Vector2.Zero, type, damage, 0f, Main.myPlayer, 0f, 0f);
						}
					}
				}
			...
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				...
				if (calamityGlobalNPC.newAI[1] >= 600f * projectileFireRateMultiplier)
				{
					...
					if (Collision.CanHit(npc.position, npc.width, npc.height, player.position, player.width, player.height))
					{
						...
						vector104.X += num942 * 5f;
						vector104.Y += num943 * 5f;
						int num947 = Projectile.NewProjectile(npc.GetSource_FromThis(), vector104.X, vector104.Y, num942, num943, type, damage + damageIncrease, 0f, Main.myPlayer, tileEnrageMult, 0f);
					...
					if (revenge)
					{
						if (lifePercentage < 50 || !player.ZoneDungeon)
						{
							...
							for (int i = 0; i < 12; i++)
							{
								int proj = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center.X, npc.Center.Y, 0f, 0f, type, damage + damageIncrease, 0f, Main.myPlayer, i * 30, ai1);
						...
						else
						{
							...
							for (i = 0; i < 4; i++)
							{
								...
								int proj = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center.X, npc.Center.Y, (float)(Math.Sin(offsetAngle) * 3f), (float)(Math.Cos(offsetAngle) * 3f), type, damage + damageIncrease, 0f, Main.myPlayer, passedVar, 0f);
								Main.projectile[proj].timeLeft = baseProjectileTimeLeft;
								passedVar += 1f;
							}
						}
			...
			if (npc.life > 0)
			{
				if (Main.netMode != NetmodeID.MultiplayerClient)
				{
					...
					if ((npc.life + num660) < calamityGlobalNPC.newAI[0])
					{
						...
						{
							NPC.NewNPC(npc.GetSource_FromThis(), (int)npc.Center.X, (int)npc.Center.Y, ModContent.NPCType<DarkEnergy>());
							NPC.NewNPC(npc.GetSource_FromThis(), (int)npc.Center.X, (int)npc.Center.Y, ModContent.NPCType<DarkEnergy2>());
							NPC.NewNPC(npc.GetSource_FromThis(), (int)npc.Center.X, (int)npc.Center.Y, ModContent.NPCType<DarkEnergy3>());
						}
					}
				}
			}
						...
						else
						{
							...
							else if (num1307 == 1)
							{
								...
								if (phase2)
								{
									...
									for (int i = 0; i < totalProjectiles; i++)
									{
										...
										Projectile.NewProjectile(npc.GetSource_FromThis(), spawnVector, velocity, type, damage, 0f, Main.myPlayer, 0f, phase3 ? 1f : 0f);
									}
								}
							}
							else if (NPC.CountNPCS(ModContent.NPCType<Bumblefuck2>()) < maxBirbs && npc.localAI[3] == 0f)
							{
								...
								for (int i = 0; i < totalProjectiles; i++)
								{
									Vector2 spawnVector = player.Center + Vector2.Normalize(new Vector2(0f, -featherVelocity).RotatedBy(radians * i)) * distance;
									Vector2 velocity = Vector2.Normalize(player.Center - spawnVector) * featherVelocity;
									Projectile.NewProjectile(npc.GetSource_FromThis(), spawnVector, velocity, type, damage, 0f, Main.myPlayer);
								}
							}
						}
					}
					...
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						...
						if (spawnFlag)
						{
							...
							if (Vector2.Distance(vector7, player.Center) > 150f)
								NPC.NewNPC(npc.GetSource_FromThis(), (int)vector7.X, (int)vector7.Y, ModContent.NPCType<Bumblefuck2>(), npc.whoAmI);
						}
					}
			...
			else if (npc.ai[0] == 5f)
			{
				...
				if (npc.ai[1] == aiGateValue - 30f)
				{
					...
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						...
						if (ai0 > 3f)
							ai0 = 3f;
						Projectile.NewProjectile(npc.GetSource_FromThis(), vector7.X, vector7.Y, 0f, 0f, ModContent.ProjectileType<BirbAuraFlare>(), 0, 0f, Main.myPlayer, ai0, npc.target + 1);
					}
				}
		...
		public static void OldDukeAI(NPC npc, Mod mod)
		{
			...
			if (calamityGlobalNPC.newAI[1] == 1f)
			{
				...
				if (calamityGlobalNPC.newAI[0] % 60f == 0f)
					SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/OldDukeHuff"), npc.position);
			...
			if (npc.ai[0] == -1f)
			{
				...
				if (npc.ai[2] == num9 - 30)
				{
					...
					SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/OldDukeRoar"), npc.position);
				}
			...
			else if (npc.ai[0] == 2f)
			{
				...
				if (npc.ai[2] % toothBallBelchPhaseDivisor == 0f)
				{
					if (npc.ai[2] % 40f == 0f && npc.ai[2] != 0f)
						SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/OldDukeVomit"), npc.position);

					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						Vector2 vector6 = Vector2.Normalize(player.Center - vector) * (npc.width + 20) / 2f + vector;
						NPC.NewNPC(npc.GetSource_FromThis(), (int)vector6.X, (int)vector6.Y + 45, ModContent.NPCType<OldDukeToothBall>());
					}
				}
			...
			else if (npc.ai[0] == 3f)
			{
				...
				if (npc.ai[2] >= num9 - 90)
				{
					if (Main.netMode != NetmodeID.MultiplayerClient && npc.ai[2] % 18f == 0f)
					{
						...
						NPC.NewNPC(npc.GetSource_FromThis(), (int)(vector.X + 900f), (int)(vector.Y - calamityGlobalNPC.newAI[2]), ModContent.NPCType<OldDukeSharkron>(), 0, 0f, 0f, npc.whoAmI, 0f, 255);
						NPC.NewNPC(npc.GetSource_FromThis(), (int)(vector.X - 900f), (int)(vector.Y - calamityGlobalNPC.newAI[2]), ModContent.NPCType<OldDukeSharkron>(), 0, 0f, 0f, npc.whoAmI, 0f, 255);
					}
				}
			...
			else if (npc.ai[0] == 4f)
			{
				...
				if (npc.ai[2] >= num10 - 60)
				{
					if (Main.netMode != NetmodeID.MultiplayerClient && npc.ai[2] % 18f == 0f)
					{
						...
						NPC.NewNPC(npc.GetSource_FromThis(), (int)(vector.X + 50f + calamityGlobalNPC.newAI[2]), (int)(vector.Y + 540f), ModContent.NPCType<OldDukeSharkron>(), 0, 0f, 0f, 1f, -12f, 255);
						NPC.NewNPC(npc.GetSource_FromThis(), (int)(vector.X - 50f - calamityGlobalNPC.newAI[2]), (int)(vector.Y + 540f), ModContent.NPCType<OldDukeSharkron>(), 0, 0f, 0f, -1f, -12f, 255);
					}
				}
			...
			else if (npc.ai[0] == 7f)
			{
				...
				if (npc.ai[2] == 0f)
				{
					...
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						Projectile.NewProjectile(npc.GetSource_FromThis(), vortexSpawn, Vector2.Zero, type, damage, 0f, Main.myPlayer, vortexSpawn.X, vortexSpawn.Y);
					}
				}
				...
				if (npc.ai[2] % toothBallSpinPhaseDivisor == 0f)
				{
					...
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						...
						int num31 = NPC.NewNPC(npc.GetSource_FromThis(), (int)vector10.X, (int)vector10.Y + 45, ModContent.NPCType<OldDukeToothBall>());
			...
			else if (npc.ai[0] == 8f)
			{
				...
				if (npc.ai[2] == num9 - 30)
				{
					...
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						...
						for (int i = 0; i < 20; i++)
						{
							...
							Projectile.NewProjectile(npc.GetSource_FromThis(), vector7.X, vector7.Y, velocityX, -velocityY, type, damage, 0f, Main.myPlayer, 0f, 0f);
						}
					}
				}
				...
				if (npc.ai[2] >= num9 - 90)
				{
					if (Main.netMode != NetmodeID.MultiplayerClient && npc.ai[2] % 18f == 0f)
					{
						...
						NPC.NewNPC(npc.GetSource_FromThis(), (int)(vector.X + x), (int)(vector.Y - calamityGlobalNPC.newAI[2]), ModContent.NPCType<OldDukeSharkron>(), 0, 0f, 0f, npc.whoAmI, 0f, 255);
						NPC.NewNPC(npc.GetSource_FromThis(), (int)(vector.X - x), (int)(vector.Y - calamityGlobalNPC.newAI[2]), ModContent.NPCType<OldDukeSharkron>(), 0, 0f, 0f, npc.whoAmI, 0f, 255);
					}
				}
				...
				if (shootProjectile)
				{
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						...
						for (int i = 0; i < 3; i++)
						{
							...
							int proj = Projectile.NewProjectile(npc.GetSource_FromThis(), vectorCenter, vector, ModContent.ProjectileType<DarkOrb>(), damage, 0f, Main.myPlayer, 8f, 0f);
						...
						{
							offsetAngle = startAngle + deltaAngle * i;
							Projectile.NewProjectile(npc.GetSource_FromThis(), vectorCenter.X, vectorCenter.Y, baseSpeed * (float)Math.Sin(offsetAngle), baseSpeed * (float)Math.Cos(offsetAngle), ModContent.ProjectileType<DarkOrb>(), damage, 0f, Main.myPlayer, 10f, 0f);
						}
					}
				}
				...
				if (Main.netMode != NetmodeID.MultiplayerClient)
				{
					...
					if (npc.localAI[0] % 60f == 0f)
					{
						...
						for (int j = 0; j < totalProjectiles; j++)
						{
							...
							Vector2 offset = velocity.RotatedBy(offsetAngle * radians, default);
							int proj = Projectile.NewProjectile(npc.GetSource_FromThis(), vectorCenter + offset, targetVector * (1f - (Math.Abs(radians) / half * scalar)), ModContent.ProjectileType<DarkOrb>(), damage, 0f, Main.myPlayer, -1f, 0f);
						}
					}
					...
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						if (npc.ai[1] == 180f)
						{
							...
							for (i = 0; i < 2; i++)
							{
								...
								Projectile.NewProjectile(npc.GetSource_FromThis(), source.X, source.Y, (float)Math.Sin(offsetAngle), (float)Math.Cos(offsetAngle), ModContent.ProjectileType<BrimstoneRay>(), 40, 0f, Main.myPlayer, 1f, npc.whoAmI);
								Projectile.NewProjectile(npc.GetSource_FromThis(), source.X, source.Y, (float)-Math.Sin(offsetAngle), (float)-Math.Cos(offsetAngle), ModContent.ProjectileType<BrimstoneRay>(), 40, 0f, Main.myPlayer, 1f, npc.whoAmI);
							}
						}
					}
					...
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						...
						else
						{
							if (npc.ai[1] == 150f)
							{
								...
								{
									offsetAngle = startAngle + deltaAngle * (i + i * i) / 2f + radialOffset * i;
									Projectile.NewProjectile(npc.GetSource_FromThis(), source.X, source.Y, (float)Math.Sin(offsetAngle), (float)Math.Cos(offsetAngle), ModContent.ProjectileType<BrimstoneTargetRay>(), 0, 0f, Main.myPlayer, 1f, npc.whoAmI);
									Projectile.NewProjectile(npc.GetSource_FromThis(), source.X, source.Y, (float)-Math.Sin(offsetAngle), (float)-Math.Cos(offsetAngle), ModContent.ProjectileType<BrimstoneTargetRay>(), 0, 0f, Main.myPlayer, 1f, npc.whoAmI);
								}
							}
						}
					...
					if (npc.type == ModContent.NPCType<Laserfish>())
					{
						...
						if (Main.netMode != NetmodeID.MultiplayerClient && npc.localAI[0] >= 120f)
						{
							...
							if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
							{
								...
								int beam = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center.X + (npc.spriteDirection == 1 ? 25f : -25f), npc.Center.Y + (Main.player[npc.target].position.Y > npc.Center.Y ? 5f : -5f), num6, num7, ProjectileID.EyeBeam, damage, 0f, Main.myPlayer, 0f, 0f);
                        ...
                        if ((Main.player[npc.target].Center - npc.Center).Length() < 350f)
                        {
                            ...
                            if (Main.netMode != NetmodeID.MultiplayerClient && npc.localAI[0] >= 180f)
                            {
                                ...
                                if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
                                {
                                    ...
                                    int beam = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center.X + (npc.spriteDirection == 1 ? 10f : -10f), npc.Center.Y, num6, num7, ModContent.ProjectileType<SulphuricAcidMist>(), damage, 0f, Main.myPlayer, 0f, 0f);
                                }
                            }
                        }
					}
