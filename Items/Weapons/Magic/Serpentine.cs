...
*/
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            int curr = Projectile.NewProjectile(source, position.X, position.Y, velocity.X, velocity.Y, ModContent.ProjectileType<SerpentineHead>(), damage, Item.knockBack, owner);

            int prev = curr;
            curr = Projectile.NewProjectile(source, position.X, position.Y, velocity.X, velocity.Y, ModContent.ProjectileType<SerpentineBody>(), damage, Item.knockBack, owner, (float)prev);
            ...
            prev = curr;
            curr = Projectile.NewProjectile(source, position.X, position.Y, velocity.X, velocity.Y, ModContent.ProjectileType<SerpentineTail>(), damage, Item.knockBack, owner, (float)prev);
