		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
			...
			if (player.Calamity().StealthStrikeAvailable())
			{
				for (float i = -6.5f; i <= 6.5f; i += 6.5f)
				{
					...
					int stealth = Projectile.NewProjectile(source, position, perturbedSpeed, type, damage, Item.knockBack, player.whoAmI, 0f, 0f);
			...
			Projectile.NewProjectile(source, position, new Vector2(velocity.X, velocity.Y), type, (int)(damage * dmgMult), Item.knockBack, player.whoAmI, 0f, 0f);
			return false;
		}
	}
}
