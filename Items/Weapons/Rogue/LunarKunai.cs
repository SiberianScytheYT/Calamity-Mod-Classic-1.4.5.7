        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
			...
			if (player.Calamity().StealthStrikeAvailable())
			{
				for (int i = 0; i < 5; i++)
				{
					...
					float x4 = vector2.X;
					float y4 = vector2.Y;
					int stealth = Projectile.NewProjectile(source, x4, y4, num148, num149, ModContent.ProjectileType<LunarKunaiProj>(), damage, Item.knockBack, player.whoAmI, 0f, 0f);
			...
			else
			{
				for (int i = 0; i < 3; i++)
				{
					...
					float x4 = vector2.X;
					float y4 = vector2.Y;
					Projectile.NewProjectile(source, x4, y4, num148, num149, ModContent.ProjectileType<LunarKunaiProj>(), damage, Item.knockBack, player.whoAmI, 0f, 0f);
				}
			}
			return false;
        }
