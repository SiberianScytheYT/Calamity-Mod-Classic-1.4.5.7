...
*/
        ...
        {
            SoundEngine.PlaySound(SoundID.NPCDeath9.WithVolumeScale(2), player.position);

            if (player.Calamity().StealthStrikeAvailable())
            {
                int p = Projectile.NewProjectile(source, position.X, position.Y, velocity.X * 1.2f, velocity.Y * 1.2f, type, damage, Item.knockBack * 3, player.whoAmI, 0f, 1f);
