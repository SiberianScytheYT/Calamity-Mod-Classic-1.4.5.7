...
*/
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            int beam = Projectile.NewProjectile(source, position.X, position.Y, velocity.X, velocity.Y, type, (int)(damage * 0.75), Item.knockBack, Main.myPlayer, 1f);
            ...
            for (int num108 = 0; num108 < Main.rand.Next(1,3); num108++)
            {
                ...
                int proj = Projectile.NewProjectile(source, vector2.X, vector2.Y, speedX2, speedY2, ProjectileID.HallowStar, damage / 3, num74, i, 0f, (float)Main.rand.Next(10));
                Main.projectile[proj].Calamity().forceMelee = true;

				speedX2 = num78 + (float)Main.rand.Next(-80, 81) * 0.02f;
				speedY2 = num79 + (float)Main.rand.Next(-80, 81) * 0.02f;
				Projectile.NewProjectile(source, vector2.X, vector2.Y, speedX2, speedY2, ModContent.ProjectileType<TerraBall>(), damage / 3, num74, i, 0f, (float)Main.rand.Next(5));
            }
            return false;
        }
