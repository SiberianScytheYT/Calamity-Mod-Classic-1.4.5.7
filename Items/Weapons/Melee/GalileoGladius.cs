		...
		private void SpawnMeteor(Player player)
        {
			if (player.whoAmI == Main.myPlayer)
			{
				if (player.Calamity().galileoCooldown <= 0)
                {
                    ...
                    CalamityUtils.ProjectileRain(player.GetSource_FromThis(), player.Center, 400f, 100f, 500f, 800f, 25f, ModContent.ProjectileType<GalileosPlanet>(), damage, 15f, player.whoAmI);
