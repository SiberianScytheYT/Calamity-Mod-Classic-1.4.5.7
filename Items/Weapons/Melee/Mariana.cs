        ...
        public override void OnHitNPC(Player player, NPC target, NPC.HitInfo hit, int damageDone)
        {
            if (target.life <= 0)
            {
                ...
                for (int num252 = 0; num252 < num251; num252++)
                {
                    ...
                    Projectile.NewProjectile(player.GetSource_ItemUse(Item), target.Center.X, target.Center.Y, value15.X, value15.Y, ModContent.ProjectileType<MarianaProjectile>(), (int)(Item.damage * (player.GetDamage(DamageClass.Generic).Additive + player.GetDamage(DamageClass.Melee).Additive - 1f)), Item.knockBack, player.whoAmI, 0f, 0f);
                }
