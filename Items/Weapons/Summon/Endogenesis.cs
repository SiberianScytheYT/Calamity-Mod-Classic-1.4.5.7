...
*/
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            if (player.altFunctionUse != 2)
            {
                ...
                source1.X = Main.mouseX + Main.screenPosition.X;
                source1.Y = Main.mouseY + Main.screenPosition.Y;
				...
				if (AttackMode == 3) //flamethrower
					dmgMult = 0.9f;
                int body = Projectile.NewProjectile(source, source1, Vector2.Zero, type, (int)(damage * dmgMult), Item.knockBack, player.whoAmI, AttackMode, 0f);
                int limbs = Projectile.NewProjectile(source, source1, Vector2.Zero, ModContent.ProjectileType<EndoCooperLimbs>(), (int)(damage * dmgMult), Item.knockBack, player.whoAmI, AttackMode, body);
