...
*/
        ...
        public override void UpdateAccessory(Player player, bool hideVisual)
        {
            ...
            if (player.whoAmI == Main.myPlayer)
            {
                ...
                {
                    Projectile.NewProjectile(player.GetSource_Accessory(Item), player.Center.X, player.Center.Y, 0f, -1f, ProjectileType<FungalClumpMinion>(), (int)(FungalClumpDamage * player.MinionDamage()), 1f, player.whoAmI, 1f, 0f);
                }
            }
            ...
            if (player.immune)
            {
                if (player.miscCounter % 8 == 0)
                {
                    if (player.whoAmI == Main.myPlayer)
                    {
						...
						Projectile rain = CalamityUtils.ProjectileRain(player.GetSource_FromThis(), player.Center, 400f, 100f, 500f, 800f, 22f, type, (int)(ProjectileDamage * player.AverageDamage()), 5f, player.whoAmI, 6, 1);
				...
				for (int l = 0; l < Main.maxNPCs; l++)
				{
					...
					if (npc.active && !npc.friendly && npc.damage > 0 && !npc.dontTakeDamage && Vector2.Distance(player.Center, npc.Center) <= auraRange)
					{
						...
						if (counter % 30 == 0)
						{
							if (player.whoAmI == Main.myPlayer)
							{
								Projectile p = Projectile.NewProjectileDirect(Entity.GetSource_FromThis(), npc.Center, Vector2.Zero, ProjectileType<DirectStrike>(), auraDmg, 0f, player.whoAmI, l);
							}
						}
					}
				}
			...
			if (counter % 480 == 0)
			{
				if (player.whoAmI == Main.myPlayer)
				{
					...
					for (int i = 0; i < FireProjectiles; i++)
					{
						...
						Projectile.NewProjectile(player.GetSource_Accessory(Item), spawn, velocity, ProjectileType<BrimstoneHellfireballFriendly2>(), (int)(ProjectileDamage * player.AverageDamage()), 5f, player.whoAmI, 0f, 0f);
					}
				}
			}
        }
