                ...
                if (ShardCountdown <= 0)
                {
                    if (player.whoAmI == Main.myPlayer)
                    {
                        ...
                        for (int i = 0; i < ShardProjectiles; i++)
                        {
                            ...
                            Projectile.NewProjectile(player.GetSource_Accessory(Item), spawn.X, spawn.Y, velocity.X / 3, velocity.Y / 2, type, (int)(damage * player.AverageDamage()), 5f, Main.myPlayer, 0f, 0f);
                        }
                    }
                }
            }
