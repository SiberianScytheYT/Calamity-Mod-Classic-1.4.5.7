    ...
    public class BossRushEvent
    {
        ...
        public static void Load()
        {
            Bosses = new List<Boss>()
            {
                ...
                {
                    int shittyStatueBoss = NPC.NewNPC(Main.player[ClosestPlayerToWorldCenter].GetSource_FromThis(),(int)(Main.player[ClosestPlayerToWorldCenter].position.X + Main.rand.Next(-100, 101)), (int)(Main.player[ClosestPlayerToWorldCenter].position.Y - 400f), type, 1);
                ...
                new Boss(ModContent.NPCType<HiveMind>(), spawnContext: type =>
                {
                    ...
                    for (int playerIndex = 0; playerIndex < Main.player.Length; playerIndex++)
                    {
                        if (Main.player[playerIndex].active)
                            Main.player[playerIndex].Spawn(PlayerSpawnContext.SpawningIntoWorld);
                    }
                ...
                {
                    Player player = Main.player[ClosestPlayerToWorldCenter];
                    int sans = NPC.NewNPC(Main.player[ClosestPlayerToWorldCenter].GetSource_FromThis(), (int)(player.position.X + Main.rand.Next(-100, 101)), (int)(player.position.Y - 400f), type, 1);
                ...
                {
                    Player player = Main.player[ClosestPlayerToWorldCenter];
                    int doctorLooneyTunes = NPC.NewNPC(Main.player[ClosestPlayerToWorldCenter].GetSource_FromThis(), (int)player.Center.X, (int)player.Center.Y - 400, type, 1);
                ...
                new Boss(ModContent.NPCType<CrabulonIdle>(), spawnContext: type =>
                {
                    ...
                    int thePefectOne = NPC.NewNPC(Main.player[ClosestPlayerToWorldCenter].GetSource_FromThis(), (int)(player.position.X + Main.rand.Next(-100, 101)), (int)(player.position.Y - 400f), type, 1);
                ...
                new Boss(ModContent.NPCType<RavagerBody>(), spawnContext: type =>
                {
                    ...
                    int ravager = NPC.NewNPC(Main.player[ClosestPlayerToWorldCenter].GetSource_FromThis(), (int)(player.position.X + Main.rand.Next(-100, 101)), (int)(player.position.Y - 400f), type, 1);
                ...
                {
                    Player player = Main.player[ClosestPlayerToWorldCenter];
                    int dukeFishron = NPC.NewNPC(Main.player[ClosestPlayerToWorldCenter].GetSource_FromThis(), (int)(player.position.X + Main.rand.Next(-100, 101)), (int)(player.position.Y - 400f), type, 1);
                    Main.npc[dukeFishron].timeLeft *= 20;
                    CalamityUtils.BossAwakenMessage(dukeFishron);
                }),
                new Boss(NPCID.MoonLordCore, spawnContext: type =>
                {
                    CalamityUtils.DisplayLocalizedText("May your skills remain sharp for the last challenges.", XerocTextColor);
                    NPC.SpawnOnPlayer(ClosestPlayerToWorldCenter, type);
                }),
                new Boss(ModContent.NPCType<AstrumDeusHeadSpectral>(), TimeChangeContext.Night, type =>
                {
                    ...
                    SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/AstrumDeusSpawn"), player.Center);
                    NPC.SpawnOnPlayer(ClosestPlayerToWorldCenter, type);
                }, usesSpecialSound: true),
                ...
                {
                    Player player = Main.player[ClosestPlayerToWorldCenter];
                    int boomerDuke = NPC.NewNPC(Main.player[ClosestPlayerToWorldCenter].GetSource_FromThis(), (int)(player.position.X + Main.rand.Next(-100, 101)), (int)(player.position.Y - 400f), type, 1);
                ...
                new Boss(ModContent.NPCType<Providence>(), TimeChangeContext.Day, type =>
                {
                    ...
                    SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/ProvidenceSpawn"), player.Center);
                    int prov = NPC.NewNPC(Main.player[ClosestPlayerToWorldCenter].GetSource_FromThis(), (int)(player.position.X + Main.rand.Next(-500, 501)), (int)(player.position.Y - 250f), type, 1);
                ...
                {
                    CalamityUtils.DisplayLocalizedText("Go forth and conquer 'til the ritual's end!", XerocTextColor);
                ...
                new Boss(ModContent.NPCType<DevourerofGodsHeadS>(), TimeChangeContext.Day, type =>
                {
                    ...
                    SoundEngine.PlaySound(new SoundStyle("CalRD/Sounds/Custom/DevourerSpawn"), player.Center);
                    NPC.SpawnOnPlayer(ClosestPlayerToWorldCenter, type);
                }, usesSpecialSound: true)
            };
            ...
            BossDeathEffects = new Dictionary<int, Action<NPC>>()
            {
                ...
                {
                    CalamityUtils.DisplayLocalizedText("Hmm? Oh, you're still alive. Unexpected, but don't get complacent just yet.", XerocTextColor);
                },
                [NPCID.CultistBoss] = npc =>
                {
                    CalamityUtils.DisplayLocalizedText("Hmm? Persistent aren't you? Perhaps you have some hope of prosperity, unlike past challengers.", XerocTextColor);
                },
                [NPCID.DukeFishron] = npc =>
                {
                    CalamityUtils.DisplayLocalizedText("Hmm? Your perseverance is truly a trait to behold. You've come further than even the demigods in such a short time.", XerocTextColor);
                },
                [ModContent.NPCType<Providence>()] = npc =>
                {
                    CalamityUtils.DisplayLocalizedText("Hmm? So you've made it to the final tier, a remarkable feat enviable by even the mightiest of the gods.", XerocTextColor);
                },
                [ModContent.NPCType<DevourerofGodsHeadS>()] = npc =>
                {
                    DropHelper.DropItem(npc.GetSource_FromThis(), npc, ModContent.ItemType<Rock>(), true);
                    BossRushStage = 0;
                    ...
                    CalamityUtils.DisplayLocalizedText("Hmm? You expected a reward beyond this mere pebble? Patience, the true reward will become apparent in time...", XerocTextColor);
                }
            };
        }
