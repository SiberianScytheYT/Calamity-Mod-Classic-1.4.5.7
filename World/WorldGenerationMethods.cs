...
using CalRD.Tiles.FurnitureAncient;
using CalRD.Tiles.Ores;
using CalRD.Tiles.SunkenSea;
using CalRD.Walls;
using CalRD.World.Planets;
...
namespace CalRD.World
{
    public class WorldGenerationMethods : ModSystem
    {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                for (int k = 0; k < (int)((double)(x * y) * frequency); k++)
                {
                    ...
                    { //snow, ice, purple ice, pink ice, red ice, astral snow, astral ice
                        if (Main.tile[tilesX, tilesY].TileType == 147 || Main.tile[tilesX, tilesY].TileType == 161 || Main.tile[tilesX, tilesY].TileType == 163 || Main.tile[tilesX, tilesY].TileType == 164 || Main.tile[tilesX, tilesY].TileType == 200 || Main.tile[tilesX, tilesY].TileType == ModContent.TileType<AstralSnow>() || Main.tile[tilesX, tilesY].TileType == ModContent.TileType<AstralIce>())
                        {
                            WorldGen.OreRunner(tilesX, tilesY, (double)WorldGen.genRand.Next(3, 8), WorldGen.genRand.Next(3, 8), (ushort)type);
                        }
                    }
        ...
        {
            ModLoader.TryGetMod("AAMod", out Mod ancientsAwakened);
        ...
        public static void ConvertFromAstral(int x, int y, ConvertType convert, bool tileframe = true)
        {
            ...
            Mod mod = ModContent.GetInstance<CalRD>();
        ...
        public static void EvilIsland(int i, int j)
        {
            ...
            for (int num39 = 0; num39 <= num38; num39++)
            {
                ...
                for (int num47 = num40 - num42 + 2; num47 <= num40 + num42 - 2; num47++)
                {
                    ...
                    Main.tile[num47, num48].Get<TileWallWireStateData>().HasTile = false;
        ...
        public static void SpecialHut(ushort tile, ushort tile2, ushort wall, int hutType, int shrineLocationX, int shrineLocationY)
        {
            ...
            for (int n = shrineLocationX - randomX - 1; n <= shrineLocationX + randomX + 1; n++)
            {
                for (int num8 = shrineLocationY - randomY - 1; num8 <= shrineLocationY + randomY + 1; num8++)
                {
                    ...
                    Main.tile[n, num8].Get<TileWallWireStateData>().Slope = 0;
            ...
            for (int num18 = shrineLocationX - randomX - 1; num18 <= shrineLocationX + randomX + 1; num18++)
            {
                ...
                while (!Main.tile[num18, num20].HasTile && num20 < Main.maxTilesY && num19 > 0)
                {
                    ...
                    Main.tile[num18, num20].Get<TileWallWireStateData>().Slope = 0;
                    num20++;
                    num19--;
                }
            }
