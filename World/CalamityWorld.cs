...
namespace CalRD.World
{
    public class CalamityWorld : ModSystem
    {
        ...
        public override void PostUpdateTime()/* tModPorter Note: Removed. Use ModSystem.PostUpdateTime */ =>  TileEntityTimeHandler.Update();
        #endregion
        ...
        public override void AddRecipes()
        {
            ...
            for (int i = 0; i < rec.Count; i++)
            {
                ...
                if (r.HasResult(ItemID.FireGauntlet))
                {
                    r.AddIngredient(ModContent.ItemType<CruptixBar>(), 5);
                }
            }
        }
        ...
        public override void ModifyWorldGenTasks(List<GenPass> tasks, ref double totalWeight)
        {
            ...
            if (WaterFromSandIndex != -1)
            {
                tasks.Insert(WaterFromSandIndex + 1, new PassLegacy("SunkenSea", delegate (GenerationProgress progress, GameConfiguration config)
                {
                    ...
                    //SunkenSea.Place(new Point(GenVars.UndergroundDesertLocation.Left, GenVars.UndergroundDesertLocation.Bottom));
                }));
            }
            ...
            if (FinalIndex != -1)
            {
                ...
                tasks.Insert(FinalIndex + 3, new PassLegacy("Rust and Dust", (GenerationProgress progress, GameConfiguration config) =>
                {
                ...
                tasks.Insert(FinalIndex + 4, new PassLegacy("Abyss", delegate (GenerationProgress progress, GameConfiguration config)
                {
                    progress.Message = "The Abyss";
                    Abyss.PlaceAbyss();
                }));

                tasks.Insert(FinalIndex + 5, new PassLegacy("Sulphur2", delegate (GenerationProgress progress, GameConfiguration config)
                {
                    progress.Message = "Finishing Sulphur Sea";
                    SulphurousSea.FinishGeneratingSulphurSea();
                }));

                tasks.Insert(FinalIndex + 6, new PassLegacy("IWannaRock", delegate (GenerationProgress progress, GameConfiguration config)
                {
        ...
        public override void ModifyHardmodeTasks(List<GenPass> tasks)
        {
            ...
            tasks.Insert(announceIndex, new PassLegacy("AstralMeteor", delegate (GenerationProgress progress, GameConfiguration config)
            {
        ...
        public override void PostUpdateWorld()
        {
                ...
                if (downedPolterghast && acidRainPoints == 1)
                {
                    ...
                    CalamityUtils.CountProjectiles(ModContent.ProjectileType<OverlyDramaticDukeSummoner>()) <= 0)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Projectile.NewProjectile(closestToAbyss.GetSource_FromThis(),closestToAbyss.Center + Vector2.UnitY * 160f, Vector2.Zero,
            ...
            int l = 0;
            float mult2 = 1.5E-05f * (float)Main.desiredWorldTilesUpdateRate;
            ...
            if (bossRushHostileProjKillCounter > 0)
            {
                ...
                if (Main.netMode == NetmodeID.Server)
                {
                    ...
                    netMessage.Write((byte)CalRDMessageType.BRHostileProjKillSync);
            ...
            if (DoGSecondStageCountdown > 0)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    if (DoGSecondStageCountdown <= 60)
                    {
                        ...
                        if (freeNPCSlots < 102)
                        {
                            CalRD.Instance.Logger.Warn("You have too many occupied NPC slots for DoG to spawn. Remove dummies or kill excessive town NPCs.");
                            DoGSecondStageCountdown = 0;
                        }
            ...
            if (ArmoredDiggerSpawnCooldown > 0)
            {
                ...
                {
                    var netMessage = Mod.GetPacket();
                    netMessage.Write((byte)CalRDMessageType.ArmoredDiggerCountdownSync);
            ...
            if (death && !CalamityPlayer.areThereAnyDamnBosses && !Main.snowMoon && !Main.pumpkinMoon && !DD2Event.Ongoing && player.statLifeMax2 >= 300 && !WorldGen.spawnEye && WorldGen.spawnHardBoss <= 0) //does not occur while a boss is alive or during certain events
            {
                if (bossSpawnCountdown <= 0 && deathBossSpawnCooldown <= 0) // Check for countdown and cooldown being 0
                {
                    if (Main.rand.NextBool(50000))
                    {
                            ...
                            if (NPC.downedMoonlord)
                            {
                                if (!downedGuardians && bossType == 0)
                                    if (Main.dayTime && (player.ZoneUnderworldHeight ||
                                        (player.ZoneHallow && player.ZoneOverworldHeight)) && Main.time < 34200) //only before 2pm
                                    {
                                        BossText();
                        ...
                        {
                            var netMessage = Mod.GetPacket();
                            netMessage.Write((byte)CalRDMessageType.BossSpawnCountdownSync);
                            netMessage.Write(bossSpawnCountdown);
                            netMessage.Send();
                            var netMessage2 = Mod.GetPacket();
                            netMessage2.Write((byte)CalRDMessageType.BossTypeSync);
                ...
                else
                {
                    ...
                    if (bossSpawnCountdown <= 0 && deathBossSpawnCooldown <= 0) // Check both cooldowns again here to avoid infinite message possibilities
                    {
                        ...
                        {
                            if (!Main.dayTime || (!player.ZoneUnderworldHeight &&
                                        (!player.ZoneHallow || !player.ZoneOverworldHeight)) || Main.time > 37800)
                                canSpawn = false;
                        }
                        ...
                        if (canSpawn)
                        {
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                ...
                                {
                                    NPC.NewNPC(player.GetSource_FromThis(),(int)player.Center.X - 300, (int)player.Center.Y - 300, bossType);
                                }
                                else
                                {
                            ...
                            {
                                var netMessage = Mod.GetPacket();
                                netMessage.Write((byte)CalRDMessageType.DeathBossSpawnCountdownSync);
                        ...
                        {
                            var netMessage = Mod.GetPacket();
                            netMessage.Write((byte)CalRDMessageType.BossTypeSync);
