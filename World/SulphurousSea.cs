...
{
	public class SulphurousSea : ModSystem
	{
							...
							if (WorldGen.InWorld(x + dx, y + dy))
							{
								if (dy >= -2 - WorldGen.genRand.Next(-2, 3))
								{
									...
									for (int y2 = 0; y2 <= extraHeight; y2++)
									{
										...
										Main.tile[x + dx, y + dy - y2].Get<TileWallWireStateData>().Slope = 0;
							...
							Math.Abs(trueX - previousX) >= ColumnMinDistanceX)
						{
							...
							if (height <= ColumnMaxHeight && height >= ColumnMinHeight)
							{
								for (int dy = 0; dy < height; dy++)
								{
									...
									Main.tile[trueX, y - dy].ClearEverything();
									Main.tile[trueX, y - dy].TileType = (ushort)ModContent.TileType<SulphurousColumn>();
									...
									Main.tile[trueX + 1, y - dy].ClearEverything();
									Main.tile[trueX, y - dy].TileType = (ushort)ModContent.TileType<SulphurousColumn>();
									Main.tile[trueX, y - dy].TileFrameX = (short)(style * 36 + 18);
									Main.tile[trueX, y - dy].TileFrameY = yFrame;
									Main.tile[trueX, y - dy].LiquidAmount = 255;
									Main.tile[trueX, y - dy].WallType = oldWall;
					...
					if (y - YStart > BlockDepth * 0.25f)
					{
						if (WorldGen.SolidTile(trueX, y - 1) && WorldGen.genRand.NextBool(10))
						{
							...
							if (dy <= StalactitePairMaxDistance && dy >= StalactitePairMinDistance)
							{
								...
								PlaceStalactite(trueX, y, height, (ushort)CalRD.Instance.Find<ModTile>($"SulphurousStalactite{type}").Type);
								if (WorldGen.SolidTile(trueX, y + dy + 1))
								{
									PlaceStalacmite(trueX, y + dy, height, (ushort)CalRD.Instance.Find<ModTile>($"SulphurousStalacmite{type}").Type);
								}
								...
								Main.tile[trueX, y - 1].Get<TileWallWireStateData>().Slope = 0;
								Main.tile[trueX, y - 1].Get<TileWallWireStateData>().IsHalfBlock = false;

								Main.tile[trueX, y + dy + 1].Get<TileWallWireStateData>().Slope = 0;
								Main.tile[trueX, y + dy + 1].Get<TileWallWireStateData>().IsHalfBlock = false;
							}
						}
					}
				}
		...
		public static void CreateBeachNearSea()
		{
			...
			WorldUtils.Find(new Point(BiomeWidth + 4, (int)GenVars.worldSurfaceLow - 20), searchCondition, out Point determinedPoint);
