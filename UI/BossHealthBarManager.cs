    ...
    */
    ...
    internal static class BossHealthBarManager
    {
        ...
        public static void Load(Mod mod)
        {
            ...
            if (!Main.dedServ)
            {
                BossMainHPBar = ModContent.Request<Texture2D>("CalRD/ExtraTextures/UI/BossHPMainBar", AssetRequestMode.ImmediateLoad).Value;
                BossComboHPBar = ModContent.Request<Texture2D>("CalRD/ExtraTextures/UI/BossHPComboBar", AssetRequestMode.ImmediateLoad).Value;
                BossSeperatorBar = ModContent.Request<Texture2D>("CalRD/ExtraTextures/UI/BossHPSeperatorBar", AssetRequestMode.ImmediateLoad).Value;

                PlatformID id = Environment.OSVersion.Platform;
                if (id == PlatformID.Win32NT)
                {
                    HPBarFont = ModContent.Request<DynamicSpriteFont>("CalRD/Fonts/HPBarFont", AssetRequestMode.ImmediateLoad).Value;
                }
                else
                {
